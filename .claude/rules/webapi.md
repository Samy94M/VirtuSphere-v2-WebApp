...
- `db_importMAC.php` validates an optional `job_id` before opening its request transaction, rechecks the running job with `FOR UPDATE`, and performs phase-2 interface, VM-state, status-event and `result_json` writes as raw prepared statements in that one transaction. No `repo_transaction()`-wrapped helper may be called from the block.
