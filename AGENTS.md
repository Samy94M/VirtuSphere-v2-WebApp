...
- Worker containers: `lib/deploy_worker.php` (`deploy-worker`) runs Ansible deploy jobs; `lib/maintenance_worker.php` (`maintenance-worker`) runs the MECM probe, retention purges and integration transition audits. Both are `--loop` CLIs that survive MySQL restarts.
- PowerShell integration lives in `Powershell-MECM/` (`mecm/` server scripts, `clients/` client phase scripts, installer). All environment specifics come from the Windows registry, not the code; scripts report heartbeats/phases to `mecm_report.php`.
...
- `scripts/`: local tooling. `lint-csp-patterns.sh` (pattern scan), `lang-audit.php` (DE/EN parity), and the SSoT drift checks `check-enum-sync.sh`, `check-php-version-sync.sh`, `check-doc-hygiene.sh` (ADR-0016). Backup/restore is `backup.sh` + `restore_test.sh` (ADR-0017, runbook `docs/operations/backup.md`).
...
- `mecm_report.php`: report channel (ADR-0018), POST-only; `action=reportPhase` (client phase events by MAC) and `action=heartbeat` (sync-loop heartbeats). Optional `X-VirtuSphere-Token` header, checked only when a token hash is configured.
