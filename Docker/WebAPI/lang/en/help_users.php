...
return [
    ...
    'usersmgmt_audit_filter' => 'The log page filters by free text, IP, category and date range; the range covers both selected days in full, counted in the configured portal timezone. Under "Diagnostic filters" it adds correlation ID, event code, object type, object ID and result. An invalid value is rejected as a field error and the query is not run at all, so the page never answers a wider question than the one in the form.',
    'usersmgmt_audit_correlation' => 'The correlation ID ties the error page, the audit row and the job of one execution together. It appears in every audit row, in the header of every job log and in the CSV export, each with a copy action, and it is searched exactly only. Under an exact search the page also lists the deploy jobs of that request. Audit rows are kept for :audit_days days, job logs only for :job_days days: an older request therefore keeps audit rows with no matching job.',
