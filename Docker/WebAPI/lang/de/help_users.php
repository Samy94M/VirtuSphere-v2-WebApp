...
return [
    ...
    'usersmgmt_audit_filter' => 'Die Protokollseite filtert nach Freitext, IP, Kategorie und Zeitraum; der Zeitraum umfasst beide gewählten Tage vollständig, gerechnet in der eingestellten Portalzeitzone. Unter „Diagnosefilter" kommen Korrelations-ID, Ereigniscode, Objekttyp, Objekt-ID und Ergebnis dazu. Ein ungültiger Wert wird als Feldfehler abgewiesen und die Abfrage gar nicht ausgeführt, damit die Seite nie eine breitere Frage beantwortet als die, die im Formular steht.',
    'usersmgmt_audit_correlation' => 'Die Korrelations-ID verbindet Fehlerseite, Auditzeile und Auftrag derselben Ausführung. Sie steht in jeder Auditzeile, im Kopf jedes Auftragsprotokolls und im CSV-Export, jeweils mit Kopieraktion, und wird ausschließlich exakt gesucht. Bei exakter Suche listet die Seite zusätzlich die Bereitstellungsaufträge dieser Anfrage. Auditzeilen werden :audit_days Tage aufbewahrt, Auftragsprotokolle nur :job_days Tage: zu einer älteren Anfrage bleiben deshalb Auditzeilen ohne zugehörigen Auftrag stehen.',
