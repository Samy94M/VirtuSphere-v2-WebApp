...
function deploy_worker_reap_stale_jobs(mysqli $db): int
{
    ...
    // Named while the row is still being written, because afterwards nothing can
    // reconstruct it. A worker that is reporting right now did NOT die: this job
    // lost its heartbeat for another reason, and its playbook may still be
    // running on the Ansible host - which is the opposite instruction to
    // "restart the service".
    $cause = integration_deploy_worker_alive_now($db)
        ? 'The deploy service is reporting, so it did not die: this job stopped writing its heartbeat for another reason (a database outage is the usual one), and its playbook may still be running on the Ansible host.'
        : 'The deploy service stopped reporting as well, so it is down or was restarted.';
