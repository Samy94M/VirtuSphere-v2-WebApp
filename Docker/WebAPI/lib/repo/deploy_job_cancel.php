...
function repo_cancel_deploy_group(mysqli $db, string $groupId, int $userId): int
{
    ...
    return repo_transaction($db, static function () use ($db, $groupId, $userId): int {
        ...
        foreach ($ids as $jobId) {
            ...
            $stmt = $db->prepare('UPDATE deploy_jobs SET status = ?, cancelled_at = NOW(), cancel_requested_at = NOW(), cancel_requested_by = ?, terminal_reason_code = ?, terminal_reason_detail = NULL, locked_at = NULL, locked_by = NULL, lock_token = NULL, worker_epoch = NULL, heartbeat_at = NULL, last_error = NULL, updated_at = NOW() WHERE id = ? AND status = ?');
...
function repo_cancel_deploy_job(mysqli $db, int $jobId, int $userId): string
{
    ...
    return repo_transaction($db, static function () use ($db, $jobId, $userId): string {
        ...
        if ($current === VIRTUSPHERE_DEPLOY_STATUS_QUEUED) {
            ...
            $reasonCode = VIRTUSPHERE_DEPLOY_TERMINAL_REASON_OPERATOR_CANCELLED;
            repo_insert_deploy_job_log_unlocked($db, $jobId, VIRTUSPHERE_DEPLOY_LOG_SYSTEM, $message);
            $stmt = $db->prepare('UPDATE deploy_jobs SET status = ?, cancelled_at = NOW(), cancel_requested_at = NOW(), cancel_requested_by = ?, terminal_reason_code = ?, terminal_reason_detail = NULL, locked_at = NULL, locked_by = NULL, lock_token = NULL, worker_epoch = NULL, heartbeat_at = NULL, last_error = NULL, updated_at = NOW() WHERE id = ?');
...
function repo_confirm_deploy_job_cancelled(mysqli $db, int $jobId, string $workerId, ?string $terminalMessage = null): bool
{
    ...
    return repo_transaction($db, static function () use ($db, $jobId, $workerId, $terminalMessage): bool {
        ...
        $stmt = $db->prepare('UPDATE deploy_jobs SET status = ?, cancelled_at = NOW(), terminal_reason_code = ?, terminal_reason_detail = ?, locked_at = NULL, locked_by = NULL, lock_token = NULL, worker_epoch = NULL, heartbeat_at = NULL, last_error = NULL, updated_at = NOW() WHERE id = ? AND locked_by = ? AND status = ?');
