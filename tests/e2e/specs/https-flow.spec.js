...
test('HTTPS flow: refusals without material, upload, overwrite guard, toggles, disable', async ({ page }) => {
  ...
  page.on('request', (request) => {
    if (request.method() === 'POST' && new URL(request.url()).pathname.endsWith('/settings.php')) {
      settingsPosts += 1;
    }
  });
  const settingsForm = (action) => page.locator(`form:has(input[name="action"][value="${action}"])`);
  const submitAndFlash = async (form, trigger = form.locator('button[type="submit"]')) => {
    await Promise.all([
      // A response header or a flash left in the previous document is not the
      // POST/redirect boundary. Wait for the new main-frame document before
      // reading its controls or starting the next action on the same page.
  ...
  const dismissConfirmation = async () => {
    const before = settingsPosts;
    await dialog.locator('button[value="cancel"]').click();
    await expect(dialog).toBeHidden();
    // close is queued separately from removal of the open attribute. The
    // shared handler clears returnValue after deciding whether to replay.
    await expect(dialog).toHaveJSProperty('returnValue', '');
    expect(settingsPosts, 'Cancel sent no settings POST').toBe(before);
  };
  ...
  await submitAndFlash(settingsForm('save_https_redirect'), dialog.locator('[data-confirm-accept]'));
  ...
  await disableButton.click();
  await expect(dialog).toBeVisible();
  await submitAndFlash(settingsForm('save_https_enabled'), dialog.locator('[data-confirm-accept]'));
