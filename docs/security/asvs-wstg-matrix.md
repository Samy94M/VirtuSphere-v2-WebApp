...
| Upload-Validierung: Zertifikat (PEM/Größe/Schlüssel-Match), Mission-Import (JSON/Version/Größe/TTL, Feldvalidierung im Dry-Run und erneut im Schreibpfad) | automatisiert | `HttpsConfigTest` (Parser + Fixtures) + E2E `https-flow.spec.js` (Nicht-PEM-Ablehnung), `mission-transfer.spec.js` (Nicht-JSON-Ablehnung), `MissionTransferRoundTripTest`, `MissionImportPreviewErrorContractTest` |
