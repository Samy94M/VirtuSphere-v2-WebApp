...
| Upload-Validierung: Zertifikat (PEM/Größe/Schlüssel-Match), Mission-Import (JSON/Tiefe/Version/Größe/TTL, Feldvalidierung im Dry-Run und erneut im Schreibpfad) | automatisiert | `HttpsConfigTest` (Parser + Fixtures) + E2E `https-flow.spec.js` (Nicht-PEM-Ablehnung), `mission-transfer.spec.js` (Nicht-JSON-Ablehnung), `MissionTransferRoundTripTest`, `MissionImportPreviewErrorContractTest` |
| Mission-Import Strukturvalidierung: Listenform per `array_is_list()`, bekannte Feldlisten als einzige Projektion, unbekannte Felder (MAC, IDs, Laufzeitstatus) wirkungslos, kaputte Form mit Position gemeldet statt still gecastet | automatisiert | `MissionTransferDocumentTest`, `MissionImportShapeContractTest` (Vorschau-Zählung gleich Schreibmenge, blockiertes Dokument schreibt nichts) |
| Mission-Import Uploadgrenzen: `MAX_FILE_SIZE` als Frueherkennung, serverseitige Größenprüfung gegen die Konstante, geordnete Puffer App < PHP-Upload < POST < nginx (beide nginx-Quellen gleich) | automatisiert | `MissionImportUploadLimitContractTest` (positiv, negativ und Zero-Match belegt), Laufzeitbeleg per `ini_get` im neu gebauten Container (`docs/QA.md`) |
| Mission-Import Sitzungsisolation: ein aktiver Handoff je Sitzung, Token-Mismatch löscht fremden Zustand nie, exakte TTL-Grenze, korrupter Zustand still bereinigt | automatisiert | `MissionImportPortalTest` (reine Statusfunktion), `MissionImportPreviewErrorContractTest` (jeder Status an einen sichtbaren Ausgang und an seine Löschregel gepinnt), E2E `mission-transfer.spec.js` |
| Mission-Import Fehlerausgabe: erwartbare Datei-/Namens-/TTL-Fehler lokalisiert und ohne Protokollzeile, unerwartete Fehler nur als Referenz plus genau eine Serverdiagnose ohne Payload, Token, Dateiname oder Temp-Pfad | automatisiert | `MissionImportPortalTest` (Sentinel-Prüfung der Logzeile), `MissionImportPreviewErrorContractTest` (Trennung erwartet/unerwartet, Argumentform der Diagnoseaufrufe) |
