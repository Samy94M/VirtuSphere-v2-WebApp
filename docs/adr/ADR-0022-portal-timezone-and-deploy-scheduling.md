...
Amendment 2026-08-25: every queued slot stopped by "Cancel group" records the
same first-request metadata and structured operator-cancellation reason as a
single queued cancellation. `last_error` remains NULL and exactly one SYSTEM
line is written per stopped slot. A running slot receives none of those fields
