...
| `unreachable` | Host aus oder nicht erreichbar | Netzwerk und Host prüfen; Auto-Retry beim nächsten Intervall; alter Cache bleibt |
...
| `ssh` | Nur lesbarer Legacy-Wert eines Abrufs vor der Herkunftstrennung; seit Etappe 8 schreibt ihn kein Producer mehr | Einen neuen Abruf starten; das Jobprotokoll des alten Abrufs nennt die tatsächliche Ursache |
...
Für unqualifizierten Altbestand gibt es keinen sicheren Backfill: Die gespeicherte Kategorie beweist nicht mehr, auf welcher Seite des früher zusammengefassten Wegs der Fehler entstand. Ein gezielter erfolgreicher Einzelabruf ersetzt die Kategorie und hebt eine Fehlpause auf. Alternativ hebt ein bewusster Einzel-Retry oder das erneute Speichern des ESXi-Zugangsdatums die historische Pause auf; die Automatik allein überspringt einen pausierten Zugang weiterhin. `last_error_category` bleibt der knappe dauerhafte Befund. Wird der Systemauftrag nach seiner Aufbewahrungsfrist gelöscht, setzt `ON DELETE SET NULL` nur `last_job_id` zurück. Es gibt bewusst keine zweite Detailkopie in `last_error_detail`: Eine solche Spiegelung bräuchte die gemeinsame Redigierung beider Secrets, das Löschen bei Erfolg und eine ausdrücklich beschlossene längere Aufbewahrung.
