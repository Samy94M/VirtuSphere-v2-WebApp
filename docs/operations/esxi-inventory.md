...
| `ssh` | Übergangswert; seit Etappe 7 schreibt ihn nur noch der Ansible-Preflight-Exitcode (Etappe 8 löst diesen letzten Producer in `ansible_preflight` auf) | Exitcode und fehlgeschlagene Komponente im Jobprotokoll prüfen |
...
Die Kategorien sind die SSoT-Liste `VIRTUSPHERE_INVENTORY_ERROR_CATEGORIES` in `lib/inventory_error_constants.php`; der bestehende Require-Pfad `lib/deploy_constants.php` lädt sie mit. Nur der exakte Code `auth` pausiert einen ESXi-Zugang; insbesondere `ansible_auth` und `ansible_authz` tun das nicht. `http` bleibt ausschließlich für lesbaren Altbestand erhalten. Seit Etappe 6 schreiben die typisierten Producer SFTP-Budgetüberschreitungen als `ansible_timeout`, andere SFTP-Fehler als `ansible_sftp` und lokale SSH-/SFTP-Voraussetzungen als `config`; gewöhnliche Exceptions mit ähnlich klingendem Text werden nicht in einen Budgettyp umgedeutet. Seit Etappe 7 qualifiziert die eine gemeinsame Funktion `ansible_connection_error_category()` (`lib/connection_errors.php`) jeden generischen Befund als Ansible-Host-Ursache, bevor der Inventarworker (SSH-/Transport-Phase) oder der SSH-Zugangstest ihn speichert: eine abgelehnte Anmeldung, ein DNS-Fehler oder eine verweigerte Verbindung zum Ansible-Host landen als `ansible_auth`/`ansible_dns`/`ansible_unreachable`, nie mehr als das generische `auth`/`dns`/`unreachable`, das dieselbe Zeile bislang wie einen ESXi-Fund aussehen ließ. `ssh` ist dadurch kein aktiver Fallback dieser beiden Producer mehr; der einzige verbliebene Schreiber ist der noch nicht umgestellte Ansible-Preflight-Exitcode (Etappe 8 löst ihn in `ansible_preflight` auf), und bestehende Altzeilen bleiben lesbar. Der technische Originaltext steht im Portal hinter „Technische Details" am Alert und zusätzlich in `logs/error.log`.

Bekannte Einschränkung bis Etappe 8: Ein Datenbankfehler, den der Worker-DB-Kanal nicht mehr auffangen kann, wird noch nicht phasenunabhängig als `worker` erkannt. Trifft er den Worker, während dieser in der SSH- oder Transportphase steht, erscheint er deshalb als `ansible_transport` und nennt den Ansible-Host, obwohl die Datenbank die Ursache ist. Der technische Text im Jobprotokoll zeigt den Unterschied sofort (eine `mysqli`-Meldung statt einer SSH-Meldung); das Containerlog des Workers trägt zusätzlich seine Zustandszeile zum Ausfall. Bei einem `ansible_transport` ohne erkennbares Netzproblem lohnt daher zuerst der Blick auf Datenbank und Workerlog.
...
- **Ein Ansible-Zugangsdatum wird weiterhin sofort geprüft** (SSH-Anmeldung plus vollständiger Preflight), denn dieser Weg läuft aus dem Portal heraus. Einen Scheduler hat der manuelle Volltest nicht, deshalb altert sein Ergebnis: Ein bestandener oder eingeschränkt bestandener Test wechselt nach `VIRTUSPHERE_ANSIBLE_PREFLIGHT_STALE_AFTER_DAYS` (aktuell 7) Tagen auf „Test veraltet". Das meldet keinen Defekt, sagt aber auch nicht mehr, dass heute alle Prüfpfade funktionieren. Ein Fehlschlag altert nie; ein bekannter Defekt darf nicht ins Neutrale verschwinden. Der Systemstatus zeigt daneben den letzten vom Worker bearbeiteten Missionsauftrag mit Ausgang, Zeit und Jobprotokoll. Er stammt direkt aus `deploy_jobs`, belegt nur seinen ausgeführten Modus und erneuert den Volltest nicht; ein nie übernommener Auftrag (`attempts = 0`) und missionslose Inventarjobs bleiben außen vor, letztere stehen auf der ESXi-Zeile.
