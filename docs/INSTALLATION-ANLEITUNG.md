...
6. Mission anlegen und Datacenter/Datastore sowie die exakte **WDS-Portgruppe (PXE)** setzen. Bleibt das Datacenter leer, muss der gewählte ESXi-Zugang genau einen unterstützten Semantik-2-Namen mit höchstens 48 Stunden alter positiver Evidenz liefern; Details und letzter Abrufversuch stehen im Systemstatus.
7. Mindestens eine VM in der Mission anlegen. Jede Netzwerkkarte braucht einen nicht leeren exakten Portgruppennamen; derselbe Name darf innerhalb einer VM nur einmal vorkommen. Für MAC-Export muss genau eine Karte exakt der WDS-Portgruppe der Mission entsprechen. Groß-/Kleinschreibung wird nicht angeglichen.
8. Deploy-Job queuen.

Wenn ein Deploy-Job nicht eingereiht werden kann, kommt der Fehler vor dem Worker zurück: fehlender Benutzer, Template-Mission, keine VMs oder unvollständige Zugänge werden backendseitig blockiert. Ein fehlendes Datacenter oder Datastore blockiert nur die Modi, die einen Standort lesen, nicht `autostart`; Portal und Backend prüfen dabei dieselbe Bedingung. Leere oder exakt doppelte allgemeine Portgruppen blockieren Create, Full, Powercycle und Export vor Remote-Arbeit, Start/Autostart warnen. Die spezielle WDS-Prüfung blockiert Full, Powercycle und Export; Create, Start und Autostart warnen allein deshalb. Der Worker prüft nach dem Claim erneut, bevor er hochlädt oder SSH öffnet.
