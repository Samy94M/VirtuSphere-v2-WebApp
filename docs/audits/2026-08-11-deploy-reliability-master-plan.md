...
8R-O darf Migrationen, Fresh-Schema, Protokollimplementierung und deaktivierte Consumer vollständig entwickeln und lokal prüfen, aber weder reale Hostmesswerte ersetzen noch einen Modus auf `pilot_remote`/`remote_enabled` setzen. Solange 8R-S nicht von einem autorisierten Betreiber mit dem versionierten Offline-Nachweisbundle ausgeführt und dessen redigiertes Ergebnis importiert wurde, bleibt der bestehende Produktionsvertrag unverändert und jede neue Remote-Ausführung technisch gesperrt. Create/Full bleiben unabhängig davon bis 14B gesperrt.
...
|---|---|---|---|---|---|---|---|---|
| 8R-O-5 Mutierende Modus-Policies | 2026-08-20 | lokal grün; alle Modi deaktiviert; 8R-S offen | Geschlossene Registry für Inventory, Export, Start, Autostart und Powercycle folgt direkt der bestehenden Ansible-Playbookfolge. Jeder Step nennt Mutation, Reconciliationowner, Callback, Recoveryevidenz und `site_acceptance_required` statt eines geratenen Runtimebudgets. Export fordert jobgebundenen MAC-Import oder Live-Inventar, Start UUID/MOID, Powerstate und Taskprüfung, Autostart materialisierte Soll-/Livepolicy plus HA-/Lizenzgate. Powercycle besteht aus den separaten Steps Powercycle und Export und kennt nur die fünf per-VM-Phasen `not_started`, `stop_requested`, `stopped_verified`, `start_requested`, `started_verified`; kein Gruppen-Blindretry. Disabled, Legacy und Rollback wählen keinen Remotevertrag; Create/Full haben keine O5-Policy. Static-Guard belegt null Aktivierungsschreiber und null Produktcaller | Zieltests `10/10`, 125 Assertions; mit O3-Pfadgegenprobe `18/18`, 173 Assertions; gezieltes PHPStan grün. Erster kanonischer Lauf `[9/9]` ausgeführt, `8 pass/1 fail`: Die Unit-/Static-Suite fand, dass der O3-Protokollleser im kanonischen `/repo`-Mount um eine Verzeichnisebene falsch auflöste. Korrigiert auf dieselbe Repo-Wurzel wie `ansible_source_dir()` und exakt statisch gepinnt. Vollständige Wiederholung `[9/9] pass`, 0 fail/skip/infrastructure_error: php-lint 482 Dateien, Unit/Static ohne Skips, PHPStan, enum-sync, bounds-sync, file-size, doc-hygiene, doc-semantics, python-client-tests. `migrate.php --check`: `pending=0` | Changelog, Deployment, QA, Deploy-Chain und konsolidierter Fachplan dokumentieren Policies, Unerreichbarkeit, fehlende Standortwerte und ausdrücklich, dass ohne 8R-S dauerhaft nichts aktiviert wird. Keine Portaltexte/i18n und keine neue ADR-Entscheidung | Nicht betroffen: keine Audit-, Joblog-, Containerlog-, Retention-, Schema-, Migrations- oder Machine-API-Wire-Änderung. Policycode führt weder DB- noch Remote-I/O aus | `2c7049f` „Etappe 8R-O-3: Protokollschema im Repo-Mount auflösen“ und `f9197ec` „Etappe 8R-O-5: Modus-Policies deaktiviert vorbereiten“, lokaler `main`; beide nicht gepusht gemäß Nutzerregel | Keine vier Aktivierungscommits, weil keine der vier realen 8R-S-Abnahmen stattfinden kann. Alle Moduszeilen bleiben `disabled`, der Worker lädt die Policy nicht und der Legacy-Produktpfad bleibt unverändert. Reale Laufzeit-/Fault-/Wirkungs-/Rollbacknachweise bleiben dauerhaft offen, bis ein autorisierter Standortoperator sie erbringt. Fremde Änderungen an `DirectoryLdapFixtureFailoverTest.php` und `EsxiTrustModeIntegrationTest.php` blieben unstaged und uncommittet |
| 8R-O-4 Reaper-/Recoverygrundlage | 2026-08-20 | lokal grün; unverdrahtet; 8R-S offen | Geschlossene Policy klassifiziert Legacy/NULL, fremde Generation, bewiesenen Nichtstart, Prepared/Active/Lost, terminales Resultat und Protocol/Manual. Nur `remote_v1` ohne Handle beziehungsweise `never_started` trägt `may_terminalize=true`; alle aktiven, verlorenen, fremden und manuellen Fälle bleiben nichtterminal und nicht cleanupfähig. Repository listet stale Kandidaten read-only, speichert Recovery idempotent, löst Workerfence/Lease ohne Statuswechsel und setzt einen gebundenen Handle nur `not_required -> pending`. Eine read-only Ersatzabfrage für den VM-Sweep schließt aktive Jobs, `recovery_requested_at` sowie Reconciliation `pending|running|manual_required` aus. Kein Deploy-/Maintenance-Reaper lädt die Module | Unit `4/4`, 21 Assertions; Static `3/3`, 16 Assertions; MySQL `2/2`, 12 Assertions, ohne Skips. DB beweist zweimalige Legacyanforderung mit `recovery_count=1`, weiterhin `running`, gelösten Locks und gesetztem `legacy_uncertain`; Remoteanforderung lässt Controller/Cleanup unverändert und setzt nur Reconciliation `pending`. Gezieltes PHPStan grün. Kanonisch `[5/5] pass`: php-lint (479 Dateien), phpstan, file-size, doc-hygiene, doc-semantics | Changelog, QA, Deploy-Chain und konsolidierter Fachplan dokumentieren Matrix, Idempotenz, VM-Sweep-Schutz und ausdrücklich fehlende Verdrahtung. Keine Portaltexte/i18n, keine ADR-Änderung | Keine Audit-, Joblog-, Containerlog-, Retention- oder Machine-API-Wire-Änderung. Das Repository schreibt ausschließlich Recoverymetadaten und löst Fence/Lease; es schreibt keinen Terminalstatus und löscht keine Evidenz. Fixtures werden vollständig zurückgebaut | `0a0d5e5` „Etappe 8R-O-4: Recoverygrundlage fail-closed vorbereiten“, lokaler `main`; 9 Dateien, 394 Einfügungen; nicht gepusht gemäß Nutzerregel | Das aktive Legacy-Reaper-/VM-Sweep-Verhalten bleibt absichtlich unverändert. Callbackraces, append-only manuelle Resolution, reale Prozess-/Hostrestart-Recovery sowie das atomare gemeinsame Umschalten von Reaper, Consumer und Sweep bleiben 8R-S beziehungsweise 13R. Fremde Änderungen an den zwei Integrationstests blieben unstaged und uncommittet |
| 8R-O-3 Deaktivierter Inventarconsumer | 2026-08-20 | lokal grün; Produktpfad unerreichbar; 8R-S offen | Checksum-geprüfter read-only Observer ergänzt das geschlossene Protokoll um Unitstatus, vorhandene Marker und einen begrenzten Outputchunk ab exakt angefordertem Rohbyte-Offset; Owner, Modus, Dateityp und Größe werden geprüft, Rotation/Truncation abgewiesen. PHP liest diese JSON-SSoT direkt und implementiert Prepare, SFTP-/Launch-/Pollnaht, persistentes Reattach, Resultat-SHA, transaktionalen Logoffset, Reconciliation und Cleanup-CAS. Jeder Write verlangt aktive Jobbindung, Worker-ID, Lock-Token, Lease-Epoch sowie gleiche Job-/Handle-/Runtime-Generation. Doppelte Chunks sind idempotent, Lücken/fremde Dokumente werden `protocol_error`/`manual_required`, Cleanup muss vor Remote-I/O `eligible -> running` reservieren. Keine Workerdatei required oder ruft den Consumer auf; `disabled` bleibt Prepare-Blocker | Python vollständig `33/33` grün, einschließlich Observeroffset/Rotationsfehler und interner `SHA256SUMS`. PHP Unit `5/5`, 19 Assertions; Static `3/3`, 28 Assertions; MySQL-Integration `3/3`, 25 Assertions, ohne Skips. Faultfälle: disabled, doppeltes Prepare, Cleanup vor Eligibility, veraltete Epoch, falscher Token, fremde Generation, DB-Zwischenwrite/Reattach, Offsetduplikat/-lücke, beschädigtes Resultat und Secret-Sentinel. Kanonischer Abschluss `[8/8] pass`: php-lint (474 Dateien), phpstan, enum-sync, bounds-sync, file-size, doc-hygiene, doc-semantics, python-client-tests; nach letzter Owner-/Mode-/Redaction-Härtung zusätzlich `[3/3] pass` php-lint/phpstan/file-size und alle Zieltests erneut grün | Runner-README, Changelog, Deployment-, QA-, Deploy-Chain- und konsolidierte Fachplandoku erklären Observer, Fencing, exactly-once Offset, Cleanup-Barriere und Unerreichbarkeit. Keine Portaltexte, daher keine i18n-Änderung. ADR-0040-Vertrag unverändert umgesetzt | DB-Joblog erhält nur über die transaktionale Offsetgrenze normalisierte und erneut geschwärzte Chunks; Integration beweist, dass der Sentinel nicht gespeichert wird. Protokollfehler persistieren Kategorie/Detail begrenzt und führen nicht zu Erfolg oder Retry. Keine Audit-/Containerlogkategorie, Retention oder Machine-API-Wire-Änderung. Testaktivierungen, Fencewerte, Jobs und Credential werden im TearDown zurückgebaut | `5279243` „Etappe 8R-O-3: Inventarconsumer fail-closed vorbereiten“, lokaler `main`; 21 Dateien, 1270 Einfügungen/6 Löschungen; nicht gepusht gemäß Nutzerregel | Keine lokale Aktivierung und kein Legacy-Fallback. Echte SSH-Trennung vor/nach Launch, Logout, User-Bus/Linger/cgroup, Worker-/Hostrestart, DB-Ausfall über den echten Transport, Ressourcenlimits, Remote-Cleanup und Sentinelprüfung in Container-/Standortartefakten bleiben 8R-S. Create/Full weiterhin bis 14B gesperrt. Fremde Änderungen an `DirectoryLdapFixtureFailoverTest.php` und `EsxiTrustModeIntegrationTest.php` blieben unstaged und uncommittet |
| 8R-O-2 Additives Schema/Fencinggrundlage | 2026-08-20 | lokal grün; vollständig fail-closed; 8R-S offen | Migration `0042_remote_execution_foundation` und Frischschema ergänzen nullable Job-Fencingfelder, die genau einmal zufällig erzeugte Runtime-Generation, Supervisorvertrag, globale Lease/Epoch mit initialer Claim-Pause, explizite Credential-/Modusaktivierungen, persistente Remote-Handles und append-only Recovery-Resolutionen. Bestehende Ansible-Zugänge werden durch die Migration, neue und auf Ansible umgestellte Zugänge atomar für exakt `create`, `powercycle`, `export`, `start`, `autostart`, `full`, `inventory` ausschließlich als `disabled`/`NULL` materialisiert. Geschlossene PHP-Registries, Datenbank-Checks und report-only Leser weisen unbekannte Vertragskombinationen ab. Backup vor Migration: `Docker/backups/db-20260820-132905.sql.gz` plus Konfigurationsarchiv. Migration lokal angewandt; `migrate.php --check`: `pending=0` | Schema-Konvergenz nach Korrektur der Zeichensatz-Introducer `[1/1] pass`; isolierter QA-Stack `[1/1] pass`. Vollrepo-Vertragstest `7/7`, 86 Assertions, ohne Skip; DB-Integration `1/1`, 25 Assertions. Kanonische Einzelgates `php-lint`, `phpstan`, `enum-sync`, `bounds-sync`, `file-size`, `doc-hygiene`, `doc-semantics` jeweils grün; der erste gemeinsame Lauf war `6/7`, weil PHPStan eine immer wahre Test-Assertion beanstandete, nach Testkorrektur kanonisch wiederholt: `[1/1] pass phpstan`. `git diff --cached --check` vor Commit grün | `docs/CHANGELOG.md`, `docs/DEPLOYMENT.md`, `docs/operations/deploy-chain.md` und der konsolidierte Fachplan dokumentieren Datenmodell, Fail-closed-Grenze und späteres gemeinsames Umschaltfenster. Keine Portaltexte und keine neue Betriebsbehauptung; ADR-0040 bleibt die Entscheidung | Keine neue Audit-, Joblog- oder Containerlogzeile. Keine Retention- und keine Machine-API-Wire-Änderung. Persistierte Lease und Claim-Pause sind in O2 bewusst noch nicht an Claim/Reaper/SSH verdrahtet; der aktive Produktpfad verhält sich weiter wie der Legacybestand | `258b6d7` „Etappe 8R-O-2: Fencinggrundlage fail-closed vorbereiten“, lokaler `main`; 14 Dateien, 754 Einfügungen/14 Löschungen; nicht gepusht gemäß Nutzerregel | Keine Standortwerte erfunden: Lease-, Heartbeat- und Grace-Werte, User-Bus/Linger/cgroup, Ressourcen-, Fault-, Mess-, Beobachtungs- und Rückbaunachweise sowie jede `pilot_remote`-/`remote_enabled`-Aktivierung bleiben 8R-S. Deshalb weder Claim-Pfad noch Reaper halb umgestellt. Fremde Änderungen an `DirectoryLdapFixtureFailoverTest.php` und `EsxiTrustModeIntegrationTest.php` blieben unstaged und uncommittet |
| 8R-O-1 Protokoll/Runner/Offline-Bundle | 2026-08-20 | lokal grün; 8R-S offen | Geschlossener Protokoll-v1-Vertrag und Golden Vectors unter `Ansible/runner/`; nur Inventory, Export, Start, Autostart und Powercycle. Launcher/Runner prüfen Run-Token, abgeleiteten Statepfad/Unitnamen, Besitzer/Modi, alle Symlink-Pfadkomponenten, Größen und SHA-256. Kein Shellstring; `systemd-run --user` mit `Type=exec`, `KillMode=control-group`, `UMask=0077`, explizitem `RuntimeMaxSec`, `--collect` und `--expand-environment=no`. Atomare Start-/Heartbeat-/Ergebnismarker, begrenzte vereinigte Ausgabe und Chunk-übergreifende Redaction. Offline-Installer überschreibt nicht; Preflight ist read-only und hat keinen Freiplatzdefault. Produktpfad unverändert | Kanonisch: `shellcheck,python-client-tests` `[2/2] pass` und `doc-hygiene,doc-semantics` `[2/2] pass`. Im QA-Linux-Image zusätzlich interne `SHA256SUMS` vollständig, `py_compile` grün und 32/32 Python-Tests ohne ResourceWarning. Positiv/negativ: unbekannte Felder, verbotener Create-Schritt, Token/Hash, Traversal, Datei-/Zwischenverzeichnis-Symlink, atomare Fehlerdatei, Loggrenze/Redaction, Doppelstartentscheidung und read-only Preflight | ADR-0040-Vertrag umgesetzt; Fachplan 24.1 nachgezogen. `offline-install.md`, `deploy-chain.md`, `DEPLOYMENT.md`, `QA.md`, `QUALITY-GATES.md` und Changelog nennen Installation, Nachweisgrenze und weiterhin aktiven Legacytransport ausdrücklich | nicht betroffen: kein DB-Schema, keine Portal-/Audit-/Joblog-/Containerlogzeile und kein Machine-API-Wire-Vertrag geändert; Runnerdateien sind ohne Produktverdrahtung unerreichbar | `17246ac` „Etappe 8R-O-1: Offline-Runner-Protokoll absichern“, lokaler `main`; nicht gepusht gemäß Nutzerregel. 19 Dateien, `git diff --cached --check` sauber, vollständiger Staged-Diff geprüft | 8R-S bleibt vollständig offen: keine Zielinstallation, kein User-Bus-/Linger-/cgroup-/Ressourcen-/Fault-/Mess-/Beobachtungs-/Rückbaunachweis und keine Modusaktivierung. Create/Full bleiben bis 14B gesperrt. Fremde Änderungen an den zwei Integrationstests blieben unstaged |
...
| 8 (Kern) Worker/Playbookgrenzen/Cancel/Pause/Logging | 2026-08-19 | grün, mit benanntem Fremdblocker; 8R abgetrennt | **Zuschnitt vorab geklärt:** Etappe 8 enthält laut Abschnitt 0 auch 8R (dauerhafter Remote-Owner). 8R-1/8R-3 verlangen einen echten Ansible-Host mit systemd-User-Bus und Linger sowie ein kontrolliertes Migrationsfenster; ein solcher Host steht nicht zur Verfügung (Nutzerauskunft: fällt erst produktiv auf). Nach Rückfrage entschieden: 8-Kern jetzt auf der heutigen SSH-Kette, 8R als eigene Teiletappe. Das bleibt planlegitim, weil Abschnitt 0 den langlebigen direkten SSH-Step erst für einen 8R-**aktivierten** Modus verbietet und kein Modus aktiviert ist; der Blocker steht als eigene Registerzeile in Abschnitt 16. **(7.6, erster Schritt vor jeder Wiringänderung)** `deploy_worker_classify_inventory_failure()` folgt jetzt der verbindlichen Reihenfolge aus Abschnitt 3: `mysqli_sql_exception` phasenunabhängig `worker` (Regel 1), `SshTransportConfigurationException` phasenunabhängig `config` (Regel 2), dann Phase; die drei Transporttypen werden nur noch in `SSH`/`SFTP`/`TRANSPORT` gelesen (Regel 6/7) und dort über die eine gemeinsame Funktion, `CONFIG` bleibt `config`/`certificate`, `MARKER` `parse`, `DB` und unbekannt `worker`. **(7.1)** Neue Phase `VIRTUSPHERE_DEPLOY_PHASE_SFTP` deckt Upload und Upload-Callbacks; Regel 9 spezialisiert dort den generischen `ansible_transport`-Fallback zu `ansible_sftp`. **(Befund 4/6)** `ansible_connection_error_category()` trägt alle drei Typen selbst, die Vorabprüfung in `credential_test_ssh_failure()` entfiel, und `credential_test_sftp_failure()` entscheidet nur noch Zugehörigkeit über das neue `ansible_connection_error_is_typed()` statt eine Kategorie zu nennen. **(7.3)** Der Preflight-Exitcode setzt vor dem Wurf `ansible_preflight` statt des Übergangswerts `ssh`; `ssh` hat damit keinen Schreiber mehr. **(7.4)** `ansible_categorize_inventory_error()` erkennt `ERROR! the playbook:`, `couldn't resolve module/action`, die fehlende Controller-Bibliothek und den `become`-Timeout als `ansible_config`; die alte breite Nadel `could not be found` ist ersetzt, weil sie auch vCenters eigene Antwort für ein fehlendes Objekt traf. **(7.5)** Pause bleibt exakt am Code `auth` (`inventory_error_pauses_credential()`), und die Once-per-Onset-Entscheidung der Auditzeile ist aus dem Catch in das prüfbare `deploy_worker_inventory_pause_onset()` gezogen. **(Strukturhunk)** `ansible_command.php` 523 → 24 Zeilen Fassade plus `ansible_command_{shell,modes,preflight}.php` (17/269/312); Owner-Registry `VIRTUSPHERE_ANSIBLE_COMMAND_MODULES`, Ausnahme aus `FILE_SIZE_ALLOWANCES` entfernt. **(7.7)** `ansible_remote_steps()` ersetzt die eine entfernte `&&`-Kette durch einen Befehl je Playbook; der Worker entscheidet vor jedem und nach jedem beendeten Schritt über `deploy_worker_assert_job_is_ours()`. `deploy_worker_finish_job()` liest bei einem Null-Treffer des Abschluss-CAS die Zeile nach und bestätigt `cancelling` unter eigenem Lock als `cancelled`, statt einen verlorenen Lock zu behaupten, den es nicht geprüft hat. Cleanup: HUP/INT/TERM-Traps je Schritt plus `deploy_worker_cleanup_remote_dir()` nach beendeter Reihe; ein Schritt ohne Rückmeldung wird bewusst stehen gelassen und das protokolliert. **(7.8)** `DeployJobOutputGate` ist die eine Stelle vor der Persistenz (Normalisierung, Redigierung, Zeilen- und Gesamtgrenze, je Kappungsart genau eine SYSTEM-Zeile); `repo_insert_deploy_job_log_unlocked()` normalisiert und kappt zusätzlich jede Zeile, weil Abschluss- und Cancelpfade dort direkt mit `mysqli` schreiben. Migration 0041 ergänzt `deploy_job_logs.stream` um `ansible` und `worker_error`; `deploy_job_log_source_label()` ist der eine Displayhelper für Tabelle und Poller-JSON. **Diffprüfung:** 57 Pfade, alle aus dieser Etappe (Arbeitsbaum war zu Sitzungsbeginn sauber, keine fremden Hunks); `git diff --check` sauber. Repository-Suche nach `VIRTUSPHERE_DEPLOY_LOG_STDOUT/STDERR` außerhalb der Konstanten: leer; nach `ansible_remote_command`: leer; nach einem hartcodierten `lib/ansible_command.php` in Static-Tests: leer (beide Leser globben jetzt die Registry) | Kanonischer Runner `scripts/check.ps1`, live gepolltes Log. **Integration-Lane 33/35 pass** (`e8-int3.log`, sauber neu aufgesetzter QA-Stack): `phpunit-unit` 150,1 s grün **ohne Skips** (die Lane fährt `--fail-on-skipped`), `phpstan` grün, `phpunit-full` 182,2 s **ohne Skips** (1236 Tests inkl. LDAPS-Fixture-Integrationstests), `lang-parity`, `enum-sync`, `bounds-sync`, `schema-convergence`, `migrate-check` (pending=0), `health-contract`, `csp-patterns`, `powershell-tests`, `ansible-*`, `qa-stack` grün, und **`e2e-portal` 560,1 s grün: die vollständige Playwright-Chromium-Suite gegen den QA-Stack**. Genau zwei rote Gates, beide derselbe Fremdbefund: `file-size` (nur `layout.php` 706/666 und `mission_transfer.php` 457, keine Etappe-8-Datei) und das daraus abgeleitete `guard-harness` (86/88 bewiesen; die zwei unbewiesenen sind `file-size.green` und `file-size.small-file-allowed`, direkt einzeln nachgewiesen mit `test-guards.ps1 -Filter file-size`: 4 proven, 2 unproven, beide mit `layout.php` als Grund). Zwei vorherige Läufe wurden verworfen und ihre roten Gates ausdrücklich als Umgebung belegt statt angenommen: ein Absturz des Docker-Daemons mitten im Browsergate (`Docker Desktop is unable to start` in zwei unabhängigen Fehlerkontexten; WSL war mit 2.7.12.0 bereits aktuell, behoben durch `wsl --shutdown` und Neustart), ein MySQL-Deadlock in `BulkVmActionsTest` gegen den gleichzeitig reapenden Wartungsworker (Pfad `vms.php`/`deploy_job_guards.php` von dieser Etappe unberührt, der Reaper nutzt `repo_reap_stale_deploy_jobs()` und nicht den geänderten Abschlusspfad; isoliert 3/3 grün, im Volllauf davor und danach grün) und vier AD-Specs an `tls_failed`, weil `/tmp/virtusphere-directory` im QA-Container `root` gehörte während die Spec als `www-data` läuft (im Container verifiziert; der Ownership-Check in `directory_tls.php:73` ist genau dafür da). Gezielte Nachweise dieser Etappe zusätzlich einzeln geführt: `DeployWorkerFailureClassificationTest`, `ConnectionErrorTest`, `ConnectionErrorMappingContractTest`, `AnsibleInventoryParseTest`, `AnsibleStepMarkerTest`, `DeployJobOutputLimitsTest`, `AnsibleCommandModuleContractTest`, `DeployWorkerModuleContractTest`, `PhaseCContractTest`, `DeployConvergenceContractTest`, `EsxiInventoryCacheTest`, `DeployCancellationStateMachineTest` (14 Tests/60 Assertions) | `esxi-inventory.md` (Fehlerbildtabelle `ssh`/`ansible_config`, SSoT-Absatz um die Reihenfolge aus Abschnitt 3 ergänzt, der „Bekannte Einschränkung bis Etappe 8"-Absatz durch die geltende Regel ersetzt), `deploy-chain.md` (Kettentabelle nennt Exitcode je Schritt und vereinigte Ausgabe; neuer Abschnitt „Playbook-Schritte, Abbruchgrenze und gespeicherte Ausgabe"), ADR-0033 (Schrittgrenze ist ein Playbook; Rennen durch konkurrierende Swaps), `docs/QA.md` (Etappe-8-Nachweisabsatz), `docs/CHANGELOG.md` (drei Einträge), `.claude/rules/webapi.md` (zwei dauerhafte Regeln: das eine Schreibtor samt Reihenfolge, und ein Befehl je Schritt samt Cleanupvertrag). DE/EN: `common.conn_ssh` und `help_system_status.esxi_cause_fix_ssh` sagen jetzt beide, dass kein Abruf den Wert mehr schreibt (die Etappe-7-Formulierung wäre durch genau diese Etappe falsch geworden); `esxi_cause_fix_ansible_config` nennt die Rechteerhöhung; `help_deploy.deploy_verbose_p1` verspricht keine Variablenanzeige mehr, neu `deploy_verbose_p2` (Redigierung und `no_log` als gestaffelte Absicherung) und `deploy_output_limits_*` mit interpolierten Konstanten; `deploy.th_stream` heißt Quelle, drei neue `log_source_*`-Labels; neu `help_deploy.deploy_cancel_p1b` (der laufende Schritt kann fertig werden, danach startet keiner mehr). `lang-audit --ci` und `check-bounds-sync --ci` grün | Joblog: neue Quellwerte `ansible`/`worker_error`, Altwerte lesbar und über denselben Helper dargestellt; zwei neue SYSTEM-Zeilen (Kappung je Art, je Auftrag genau eine) und je eine Zeile für „Remote-Verzeichnis stehen gelassen"/„konnte nicht entfernt werden". Auditvertrag unverändert: dieselbe Once-per-Onset-Zeile am exakten Code `auth`, keine neue Kategorie, kein neuer Producer. Containerlog unverändert (die Ausfallzeile des Kanals bleibt redigiert und einmalig). Machine-API-Wire-Contract unverändert geprüft: keine neue Kategorie erreicht ihn, `db_importMAC.php` und die `mecm-*`-Antworten sind nicht berührt. Schema: additive Migration 0041 plus order-exakter Spiegel in `struktur.sql` und in der historischen `CREATE`-Definition (dieselbe Konvergenzform wie bei `deploy_jobs.status`), `check-enum-sync.sh` prüft die Spalte jetzt mit | `364de44` „Etappe 8 (Kern): Ein Abbruch greift zwischen zwei Playbooks, und ein Datenbankausfall beschuldigt nicht mehr den Ansible-Host", `origin/main` (`github.com/Samy94M/VirtuSphere-v2-WebApp`), `3187391..364de44`, Upstream-Hashgleichheit explizit bestätigt (lokal = remote = `364de44e66e824dc015e5c7ab0107ad3b8531217`), Push 2026-08-19T13:04:22Z. 57 Pfade über explizite Pfadangaben gestaged, `git diff --cached --check` sauber, keine Hunkchirurgie nötig, weil der Arbeitsbaum zu Sitzungsbeginn sauber war und alle 57 dieser Etappe gehören. Bewusst nicht enthalten und unangetastet: die während dieser Etappe im parallelen Arbeitsbaum entstandenen fremden `isset($this->db)`-Guards in `tests/Integration/DirectoryLdapFixtureFailoverTest.php` und `tests/Integration/EsxiTrustModeIntegrationTest.php` | **Fremdblocker:** `file-size` ist bereits auf `HEAD` rot, verursacht von `3187391` (Mission-Import): `lib/layout.php` 706 gegen Kontingent 666 und `lib/mission_transfer.php` 457 ohne Kontingent, beides gegen `git show HEAD:` verifiziert und ohne Bezug zu dieser Etappe. Etappe 8 hat jede eigene Datei unter der Grenze gehalten und dafür `deploy_job_output.php` und `deploy_job_log_spool.php` abgetrennt; das Kontingent von `migrate.php` wurde wie bei jeder Migration zuvor auf die neue Zeilenzahl gesetzt. **Zweiter Fremdbefund:** `DirectoryLdapFixtureFailoverTest` erzeugt ohne laufende LDAP-Fixture vier Errors statt vier Skips (`tearDown()` greift nach dem Skip auf ein nicht initialisiertes `$db` zu); in der Integration-Lane mit Fixture laufen die Tests durch. Beide stehen als eigene Registerzeilen in Abschnitt 16. **Bewusst nicht in dieser Etappe:** 8R vollständig, der Secret-Sentinel bis Browser-DOM und Rohdownload (10A), Step-Marker als Phasenüberschrift (13), Terminalergebnis-SSoT (10B) |
| 8R-O-0 Planharmonisierung/Offline-Audit | 2026-08-20 | lokal grün; 8R-S offen | Infrastrukturentscheid in Abschnitt 0 und Fachplan 0.1/24: Implementierung und Standortabnahme sind getrennt; `pilot_remote`/`remote_enabled` bleiben ohne importierte Standortevidenz unerreichbar. Read-only Audit auf `main`/`039f49c`: Migration Source/DB `0041`, `migrate --check pending=0`, keine queued/running/cancelling Jobs, beide Worker healthy, kein Ansible-Credential, kein `Ansible/runner`, 502,55 GiB lokaler Freiplatz. Zielhostwerte, Remote-Freiplatz und Messreihen ausdrücklich nicht ersetzt | `check-doc-hygiene.sh` grün. `check-doc-semantics.sh` meldet 14 bereits auf HEAD vorhandene Phantompfade; keine Meldung nennt einen 8R-O-0-Pfad. Vollständiger Staged-Diff geprüft, `git diff --cached --check` sauber | ADR-0040 neu; ADR-Index, Masterplan und konsolidierter Fachplan harmonisiert. Betriebs-/Hilfe-/Changelogtexte nicht betroffen: noch keine Laufzeit-, Portal- oder Nutzerverhaltensänderung | nicht betroffen: keine Runtime, kein Schema, kein Audit-/Job-/Containerlog und kein Wire-Vertrag geändert | `26796f0` „Etappe 8R-O-0: Offline-Implementierung und Standortabnahme trennen"; lokaler `main`, nicht gepusht gemäß ausdrücklicher Nutzerregel | 8R-S bleibt mangels Zielzugang offen. Fremde Änderungen an `DirectoryLdapFixtureFailoverTest.php` und `EsxiTrustModeIntegrationTest.php` nicht gestaged/committet |
| 9 Anzeige/Links/Zugangstest | 2026-08-21 | grün | **(8.1)** Alle neun `common.conn_ansible_*`-Sätze gegen die jetzt vollständig verdrahteten Anzeigezweige gelesen; drei Abweichungen korrigiert, die erst aus der konkreten Darstellung entstehen: `conn_ansible_sftp` nennt die bestandene SSH-Anmeldung (die Schichtaussage fehlte, obwohl genau sie den Unterschied zu `ansible_auth` macht), `esxi_cause_fix_ansible_auth` nennt zusätzlich den Kontostatus und sagt, dass der Host nicht verrät, welche der vier Ursachen es war, `esxi_cause_fix_ansible_authz` nennt die SSH-/SFTP-Policy des Hosts. `common.conn_ssh` sagt jetzt selbst, wodurch ein neuer Abruf den Altwert ersetzt; die Aussage stand nur in `esxi_cause_fix_ssh`, während die Karte allein den Satz zeigt. **(8.2)** `esxi_inv_p2` beschränkt die Pause auf eine abgelehnte ESXi-/vCenter-Anmeldung und sagt ausdrücklich, dass Ansible-Auth nicht pausiert; `esxi_cause_p1` trennt den ungegateten Satz von den zwei Bedingungen des Jobprotokoll-Links und nennt den neuen Ansible-Link; die Timeout-Abhilfe nennt die vier lesenden Inventarursachen und weiterhin keine VM-Erstellung, Festplattengröße oder Adoption. **(8.3)** Neuer Schlüssel `system_status.inv_open_ansible_status`; die ESXi-Karte rendert „Ansible-Status öffnen" genau dann, wenn `inventory_error_is_ansible()` gilt, ohne eigene Berechtigung, gebaut über `system_status_url(VIRTUSPHERE_SYSTEM_STATUS_ANCHOR_ANSIBLE)`. Joblog-Link bleibt getrennt an `deploy.run`, Volltest und Zugangsdaten-Aktion im Ansible-Abschnitt bleiben an `credentials.manage` (unverändert, geprüft). **(8.4)** `credential_test_ssh_failure()` liefert die `ansible_*`-Codes seit Etappe 7 und bleibt unverändert; der Flash zeigt sein redigiertes Detail direkt und verweist auf kein Jobprotokoll (in `portal/credentials.php` geprüft, unverändert). Der Ownership-Kommentar in `credentials_test_message.php` ist berichtigt: Ein Testergebnis ist die Vereinigung aus den wenigen `VIRTUSPHERE_CREDENTIAL_TEST_*` und den gemeinsamen Inventarcodes; `connection_error_message()` bleibt die einzige SSoT, eine zweite Mappingtabelle bleibt verboten. **Vollständigkeitsabgleich:** Repository-Suche nach allen `connection_error_message()`-Aufrufern (drei: Inventarkarte, Hilfetabelle, Testflash, alle geprüft), nach handgeschriebenem `system_status.php#` (keiner), nach weiteren Pausebehauptungen in DE-Texten (fünf Fundstellen, alle konsistent). Kein `esxi_cause_fix_<code>` fehlt, weil die Hilfetabelle über `VIRTUSPHERE_INVENTORY_ERROR_CATEGORIES` läuft. **Browsergegenprobe, die kein Test gefunden hätte:** Mit einem gesetzten `ansible_auth`-Zustand am lokalen Stack gerendert und angesehen. Der Link stand direkt hinter dem Jobprotokoll-Link, nur durch ein Leerzeichen getrennt und beide unterstrichen; das las sich als ein einziger langer Link, weil es an dieser Stelle bis zu diesem Zweig nie mehr als einen gab. Beide Folgeaktionen liegen jetzt in der bestehenden gemeinsamen `.alert-actions`-Zeile unter dem Satz und tragen den Mittelpunkt, mit dem diese Datei schon Angaben nebeneinander trennt; eine einzelne Folgeaktion bekommt keinen Trenner (`testASingleFollowUpGetsNoSeparator`). Danach in hell und dunkel sowie im ESXi-Zweig ohne Ansible-Link erneut gerendert; der lokale Zustandssatz wurde anschließend auf seinen Ausgangswert zurückgesetzt. Die Gegenprobe blieb nicht ad hoc: `tests/e2e/specs/system-status.spec.js` schreibt denselben dauerhaften Befund am Ende seines ESXi-Karten-Tests auf `ansible_auth` um und prüft im Browser die Geometrie statt der Markup-Form (beide Links sichtbar, mindestens 10 px waagerechter Abstand zwischen ihren Bounding-Boxen), danach den Klick auf `system_status.php#ansible` und die Sichtbarkeit genau dieses Abschnitts. Der Abstand ist die Aussage, weil das Markup schon richtig war, als der Fehler noch da war. **Diff:** 16 Pfade, davon eine neue Datei; kein Strukturhunk nötig (`system_status_esxi_panels.php` 368 Zeilen, unter der ADR-0006-Grenze) | Zieltests `60/60`, 606 Assertions (`SystemStatusPanelBranchTest`, `ConnectionErrorTest`, neu `SystemStatusDeepLinkContractTest`, dazu die Nachbarwächter `InventoryErrorVocabularyContractTest`, `ConnectionErrorMappingContractTest`, `CredentialsTestActionTest`). **Vier Mutantenproben, jede rot in genau der erwarteten Menge:** `:host` in `conn_ansible_sftp` lässt `testNoAnsibleSentenceInterpolatesTheEsxiHost` und den gerenderten Kategorienlauf fallen; `if (false)` statt `inventory_error_is_ansible()` lässt die zwei Linkzweige fallen; eine umbenannte `#ansible`-Section-Id lässt `testEveryAnchorConstantIsRenderedAsASectionId` fallen und benennt die Konstante; der entfernte Trenner lässt den E2E-Abstandstest fallen. **E2E:** `tests/e2e/specs/system-status.spec.js` `9 passed (28,2s)` gegen den laufenden Stack, Chromium. **Kanonisch `scripts/check.ps1 -Lane Fast`: `[28/28] pass`, `28 pass, 0 fail, 0 infrastructure_error, 0 not_applicable, 0 skip => Exit 0`** über den eingefrorenen Endstand, darunter `php-lint` (491 Dateien), `phpunit-unit` 158,8 s **ohne Skips** (die Lane fährt `--fail-on-skipped`), `phpstan`, `lang-parity`, `enum-sync`, `bounds-sync`, `file-size`, `doc-hygiene`, `doc-semantics`, `csp-patterns`, `powershell-tests`. Zwei frühere Vollläufe waren ebenfalls `28/28`, wurden aber verworfen, weil danach noch der Layout-Fix und die Doku-Nachträge kamen; nur der dritte Lauf ist der Nachweis | `help_system_status` (DE/EN: `esxi_inv_p2`, `esxi_cause_p1`, `esxi_cause_fix_ansible_auth`/`_authz`/`_timeout`), `common` (DE/EN: `conn_ansible_sftp`, `conn_ssh`), `system_status` (DE/EN: neuer Linktext). `docs/operations/esxi-inventory.md` führt beide Links der Karte mit ihrer jeweiligen Bindung als Tabelle und sagt, warum der Reparaturlink ungegated ist und warum der Testflash sein Detail direkt zeigt. `docs/QA.md` erklärt den neuen Deep-Link-Wächter, die zwei Etappe-9-Vollläufe und warum die E2E-Aussage eine Geometrie ist. `docs/operations/troubleshooting.md` bekommt eine eigene Symptomzeile „ESXi-Inventarfehler nennt den Ansible-Host": wohin der Link führt, dass der ESXi-Zugang dadurch nicht pausiert und dass der Host bei `ansible_auth` nicht sagt, welche der vier Ursachen es war. `.claude/rules/portal.md` bekommt zwei Dauerregeln: `system_status_url()` als dritter Fragment-Builder samt Wächter und Ausnahmen, und dass zwei nebeneinanderstehende Links getrennt gehören, weil weder ein Katalog- noch ein Render-Walk das findet. `docs/CHANGELOG.md` ein Eintrag. Keine neue ADR: Der Link nutzt eine bestehende Seite, eine bestehende Berechtigung und die bestehende Anker-SSoT; ADR-0014 und ADR-0006 bleiben unverändert gültig. Der Gesamtabgleich von `esxi-inventory.md` (Überschrift „nie blockierend", `logs/error.log`-Zusage, Altbestandshinweis) bleibt planmäßig Etappe 10 | nicht betroffen: keine neue Audit-, Joblog-, Container- oder Fehlerlogzeile, keine Retention-, Schema-, Migrations- und keine Machine-API-Wire-Änderung. Der neue Link ist ein `GET` auf dieselbe Seite, erzeugt also weder POST-Guard noch CSRF- noch Auditpfad; `portal_guard_post`-, Confirm- und Action-Inventare sind unberührt. RBAC ausdrücklich geprüft: `deploy.run` am Jobprotokoll und `credentials.manage` an Volltest/Zugangsdaten unverändert, der neue Link bewusst ungegated mit dem Ziel auf derselben autorisierten Seite | PLATZHALTER-COMMIT | Bewusst nicht in dieser Etappe: der vollständige `esxi-inventory.md`-Gesamtabgleich und der Deploy-QoL-Hunk (10), Joblog-Tail/Rohdownload (10A), Terminalergebnis (10B). **Bestandsartefakt, nicht von dieser Etappe:** `SystemStatusPanelBranchTest` meldet den jeweils zuerst ausgeführten Test als risky („did not remove its own error handlers"), auch bei ausschließlich vorbestehenden Tests; im Suite-Lauf tritt es nicht auf. **Fremde Hunks:** `docs/audits/2026-08-11-deploy-reliability-master-plan.md` trug beim Sitzungsbeginn bereits die 14D-Planerweiterung einer früheren Sitzung; nur die eigenen Etappe-9-Hunks (diese Zeile und vier Befundnachweise) wurden gestaged |
...
| Ansible-Code nennt ESXi-Host | 6–9 | Geschlossen in 9 an der Anzeige, nachdem 6/7 die eigenen Codes erzeugt hatten. Kein `common.conn_ansible_*`-Satz interpoliert `:host`: `ConnectionErrorTest::testNoAnsibleSentenceInterpolatesTheEsxiHost` ruft jeden Code, den `inventory_error_is_ansible()` erkennt, in beiden Locales mit `['host' => 'esxi-should-not-appear']` auf und verbietet Wert, rohen Platzhalter und Schlüssel-Echo; die Gegenrichtung verlangt, dass ein ESXi-Code den Host weiterhin nennt, damit die Prüfung eine echte Unterscheidung testet. `SystemStatusPanelBranchTest::testEveryCategoryDecidesSentenceOriginAndPermissionSeparately` wiederholt das am gerenderten Alert für die gesamte Kategorienliste. Beide fallen gegen den Mutanten `conn_ansible_sftp` mit `:host`. Zusätzlich trägt die Karte jetzt den Weg zum Ansible-Abschnitt, statt den Verursacher nur zu benennen |
| Ansible-Auth pausiert ESXi | 7–8, Integration | |
| Authz wird zu Passwortfehler | 6–9 | `ansible_authz` ist seit 7 ein eigener Code mit eigener Abbildung (`ConnectionErrorTest::testAnsibleConnectionErrorCategoryQualifiesEveryGenericFinding`); Etappe 9 schließt die Anzeigeseite. Der Satz sagt „Die Anmeldung am Ansible-Host stand, aber die Sitzung oder Aktion war nicht erlaubt", nennt also die bestandene Anmeldung ausdrücklich, und die Abhilfe nennt jetzt zusätzlich die SSH-/SFTP-Policy des Hosts statt nur die Sitzungsrechte. Umgekehrt behauptet `conn_ansible_auth` weiterhin nicht, das Secret sei falsch; die Abhilfe zählt alle vier möglichen Ursachen auf (Benutzername, Secret, Anmeldeverfahren, Kontostatus) und sagt, dass der Host nicht verrät, welche es war |
| eigene Budgets werden aus Text geraten | 6–8 | |
| SFTP-Operations-/Gesamtbudget unvollständig | 6 | |
| Preflight heißt Verbindungsfehler | 8–9 | Etappe 8 hat den Schreiber umgestellt (Preflight-Exitcode setzt `ansible_preflight`, `ssh` hat keinen Schreiber mehr); Etappe 9 prüft die Anzeige. Der Satz trennt die Schichten ausdrücklich („Der Ansible-Host ist erreichbar, aber die Vorprüfung der Toolchain ist fehlgeschlagen"), die Abhilfe führt in das Jobprotokoll zur fehlgeschlagenen Komponente, und der Kategorienlauf über `VIRTUSPHERE_INVENTORY_ERROR_CATEGORIES` belegt, dass die Karte genau diesen Satz rendert und keinen Verbindungssatz. Der synchrone Test behält daneben seine genauere eigene Formulierung mit Komponentennamen (`credentials.test_err_preflight_component`), weil er weiß, welcher Schritt lief |
...
| Joblog-Link verschweigt RBAC/Retention | 9–10 | Etappe-9-Anteil erledigt, Etappe 10 prüft ihn im Gesamtabgleich erneut mit. `help_system_status.esxi_cause_p1` nennt in DE und EN beide Bedingungen statt nur der Aufbewahrung und trennt sie ausdrücklich von dem, was ungegated bleibt: Kategorie und Satz sind für jeden Betrachter sichtbar, der Link verlangt einen noch aufbewahrten Auftrag **und** das Recht, Bereitstellungsaufträge einzureihen und deren Protokolle einzusehen. `docs/operations/esxi-inventory.md` führt beide Links der Karte mit ihrer jeweiligen Bedingung als Tabelle. `SystemStatusPanelBranchTest::testEveryCategoryDecidesSentenceOriginAndPermissionSeparately` fährt jede Kategorie mit und ohne `deploy.run` und verlangt den Satz in beiden Zuständen, den Joblog-Link nur im ersten |
...
| Klassifikator ohne phasenunabhängige `mysqli_sql_exception`-Regel | 8 (Kern) | Behoben als erster Schritt der Etappe, vor jeder weiteren Wiringänderung. `deploy_worker_classify_inventory_failure()` prüft `mysqli_sql_exception` vor allem anderen und liefert phasenunabhängig `worker`. `DeployWorkerFailureClassificationTest::testADatabaseFailureIsOursInEveryPhase` beweist es in allen sechs Phasen mit einer Meldung („MySQL server has gone away"), deren jedes Wort eine Nadel des generischen Klassifikators ist; die Gegenrichtung `testDatabaseSoundingTextFromTheRemoteHostIsNotOurs` verlangt, dass derselbe Text als gewöhnliche `RuntimeException` weiterhin `ansible_transport` bleibt. Die „bekannte Einschränkung"-Absätze in `esxi-inventory.md` und `deploy-chain.md` sind entfernt und durch die geltende Regel ersetzt |
| `ansible_connection_error_category()` behandelt `SshTransportConfigurationException` nicht selbst | 8 (Kern) | Behoben. Die gemeinsame Funktion bildet jetzt alle drei Transporttypen selbst ab; die vorgelagerte Prüfung in `credential_test_ssh_failure()` ist entfallen, sodass sie tatsächlich die eine Stelle ist. `ConnectionErrorTest::testTheSharedClassifierCallsALocalMisconfigurationOurs` prüft beide Richtungen (Funktion direkt und der Aufrufer ohne eigene Vorabprüfung) |
| Budgettyp wird phasenunabhängig `ansible_timeout` | 8 (Kern) | Behoben. Die drei Transporttypen werden nur noch innerhalb der Phasen `SSH`/`SFTP`/`TRANSPORT` gelesen, und dort über dieselbe gemeinsame Funktion; `CONFIG` bleibt `config`, `MARKER` bleibt `parse`, `DB` bleibt `worker`. `testABudgetTypeOutsideTheRemoteLegsKeepsItsPhaseAnswer` pinnt die beiden Widerspruchsfälle, die Abschnitt 3 Punkt 6 nennt |
| `credential_test_sftp_failure()` dupliziert die Typabbildung zur Hälfte | 8 (Kern) | Entschieden und behoben: die gemeinsame Funktion trägt alle drei Typen, dieser Pfad entscheidet nur noch, **welche** Fehler ihm gehören, und benennt keine Kategorie mehr selbst. Sein eigener `SftpTransportFailed` und ein untypisierter Überraschungsfehler bleiben bewusst `VIRTUSPHERE_CREDENTIAL_TEST_SFTP`, weil `credentials.test_err_sftp` sagt, dass Anmeldung und Toolchain bereits in Ordnung sind und auf SFTP-Subsystem und /tmp zeigt, was der generische `ansible_sftp`-Satz nicht kann. `ConnectionErrorTest::testTheSftpProbeOwnsItsOwnFailureButNoOtherMapping` prüft das Verhalten und verbietet zusätzlich jede Kategoriekonstante im Funktionskörper. Die Zugehörigkeitsfrage selbst hat mit `ansible_connection_error_is_typed()` eine eigene SSoT, die `ConnectionErrorMappingContractTest::testTheTypedPredicateKnowsExactlyTheTypesTheMappingDecidesByType` beidseitig gegen die `instanceof`-Liste der Abbildung hält |
...
| entfernte `&&`-Playbookkette bietet entgegen ADR-/Hilfetext keine lokale Abbruchgrenze | 8 (Kern) | Behoben. `ansible_remote_steps()` liefert geordnete Schrittdeskriptoren, jeder Playbook-Schritt ist ein eigener entfernter Befehl, und vor jedem sowie nach jedem beendeten Schritt entscheidet `deploy_worker_assert_job_is_ours()`. `AnsibleStepMarkerTest::testEveryPlaybookIsItsOwnBracketedStepInOrder` hält Reihenfolge und Isolation (kein Schritt enthält das Playbook eines anderen) |
| letzter Schritt, Cancel und Erfolg können ohne gemeinsamen CAS widersprüchlich finalisieren | 8 (Kern), 10B | Etappe-8-Anteil behoben: `repo_finish_deploy_job()` war bereits ein CAS aus `running` mit eigenem Besitz, aber ein Nulltreffer endete in einer Logzeile, die einen verlorenen Lock behauptete. `deploy_worker_finish_job()` liest jetzt die Zeile nach und bestätigt bei `cancelling` unter eigenem Lock `cancelled`, mit der Aussage, dass die Arbeit des laufenden Schritts ausgeführt wurde. `DeployCancellationStateMachineTest` deckt beide Rennrichtungen und den fremden Terminalzustand ab. Offen bleibt der 10B-Anteil (Terminalergebnis-SSoT, Cancelmetadaten) |
...
| `2>&1` wird im Portal fälschlich als trennbarer stdout-/stderr-Stream dargestellt | 8 (Kern), 13 | Etappe-8-Anteil behoben: Migration 0041 ergänzt `deploy_job_logs.stream` um `ansible` und `worker_error`, neue Zeilen tragen diese Werte, und `deploy_job_log_source_label()` stellt Alt- wie Neuwerte als eine Ansible-Ausgabe dar (Tabelle und Poller-JSON lesen denselben Helper). Der Spaltenkopf heisst jetzt Quelle statt Stream. Offen bleibt der 13er-Anteil (Filter-/Follow-UI) |
| Step-Marker werden nicht als aktuelle Phase/Phasenüberschrift genutzt | 8 (Kern), 13 | Etappe-8-Anteil: Die Marker bleiben technische SSoT und werden je Schritt gesetzt; der Worker leitet den fehlgeschlagenen Schritt nicht mehr aus dem Markerstrom ab, sondern aus dem Deskriptor, sodass auch ein Transportfehler vor dem ersten Marker seinen Schritt benennt. Die Anzeige als Phasenüberschrift und aktuelle Phase gehört zu Etappe 13 und bleibt offen. **Restprüfung 2026-08-20:** Der Leser wird nicht isoliert vorgezogen. Der aktuelle Joblogpfad liest initial den Anfang mit Limit 1000 und pollt höchstens 500 Zeilen ohne den Tail-/Drain-/`caught_up`-/Rohdownloadvertrag aus 10A; Phasenfilter, Follow und der Terminal-Presenter aus 10B/13 fehlen ebenfalls. Eine Teilintegration würde daher genau vor ihrer vollständigen Konsumentenkette erneut umgebaut. `AnsibleCommandModuleContractTest` beweist bis Etappe 13 genau die eine Definition und keinen Produktivaufrufer; Parser-Surface und Marker-Unit-Tests halten die künftige SSoT zugleich ausführbar. Der isolierte Lauf deckte außerdem zwei von der Suitenreihenfolge verdeckte Require-Lücken der Step-Fassade auf (`credentials.php` für `VIRTUSPHERE_ESXI_TRUST_FILE`, `errors.php` für `virtusphere_correlation_id()`); beide Owner lädt `ansible_command_modes.php` jetzt selbst. Marker-, Modul-, Konvergenz- und CLI-Closure-Verträge 29/29 (283 Assertions), PHPStan ohne Befund, `file-size` `[1/1] pass` |
| Jobausgabe hat keine zentrale ANSI-/Steuerzeichen-, Zeilen- und Gesamtvolumengrenze | 8 (Kern) | Behoben. `DeployJobOutputGate` (`lib/deploy_job_output.php`) ist die eine Stelle vor der Persistenz: gültiges UTF-8, ANSI-/C0-/DEL-Entfernung mit erlaubtem Tab, Zeilenkappung auf `VIRTUSPHERE_DEPLOY_OUTPUT_LINE_MAX_BYTES` und Gesamtvolumen auf `VIRTUSPHERE_DEPLOY_OUTPUT_JOB_MAX_BYTES`, je Kappungsart genau eine SYSTEM-Zeile. `DeployJobOutputLimitsTest` (8 Tests) deckt Farbsequenzen, OSC, ungültiges UTF-8, Mehrbyte-Schnittgrenze, beide Einmal-Meldungen und das Verstummen nach dem Gesamtbudget ab |
| harter Abbruch/Hostverlust kann entferntes `accounts.yml`-/Jobmaterial trotz EXIT-Trap verwaisen lassen | 8 (Kern) | Teilweise behoben, Rest ausdrücklich 8R. Ein `EXIT`-Trap ist bei einem Befehl je Playbook nicht mehr möglich (er würde nach dem ersten Schritt löschen); jeder Schritt traegt jetzt Traps auf HUP/INT/TERM, und `deploy_worker_cleanup_remote_dir()` räumt nach einer beendeten Reihe zusaetzlich selbst auf. Ein Schritt, der nicht zurückgemeldet hat, wird bewusst stehen gelassen und das im Protokoll gesagt, statt unter einem laufenden Playbook zu löschen. Der vollständige Cleanupzustand (bewiesenes Controllerende, Diagnosefrist, Cleanup-Lease) gehört zu 8R und bleibt offen |
| Hilfe behauptet für `-vvv` Variablenanzeige oder überdehnt `no_log` als Vollschutz | 8 (Kern) | Behoben. `help_deploy.deploy_verbose_p1` beschreibt nur noch eine Detailstufe ohne Inhaltszusage, `deploy_verbose_p2` nennt Redigierung und `no_log` ausdrücklich als gestaffelte Absicherung ohne Garantie, und der Formularhinweis verspricht keine Variablenanzeige mehr. Neu `deploy_output_limits_p1` mit interpolierten Konstanten; `check-bounds-sync.php --ci` grün |
| Secret-Redigierung ist nicht von `accounts.yml` bis Browser/Rohdownload/Audit-/PHP-Log bewiesen | 8 (Kern), 10A | Etappe-8-Anteil behoben: Die Redigierung sitzt jetzt am Schreibtor selbst statt an einzelnen Aufrufstellen, läuft vor der Spool (eine Störung kann kein Geheimnis zwischenlagern) und nach der Normalisierung (eine Farbsequenz mitten im Wert hebelt sie nicht aus). `DeployJobOutputLimitsTest` verfolgt ein synthetisches Sentinel in Klartext-, URL- und farbunterbrochener Form bis in die Schreibaufrufe. Der vollständige Weg bis Browser-DOM und Rohdownload gehört zu 10A und bleibt offen |
...
| `file-size`-Gate ist bereits auf `HEAD` rot, unabhängig von dieser Etappe | Verursacher `3187391` (Mission-Import), nicht Etappe 8 | Erledigt im Etappe-8-Rest. Der fachliche `layout.php`-Split wurde vorgezogen, weil ein alleiniger Split von `mission_transfer.php` den echten Gate weiter an der 666-Zeilen-Decke hätte scheitern lassen und eine höhere Ausnahme nach ADR-0006 verboten ist: Chrome-Fassade 220 Zeilen, Response/Flash 215 und Status-/Label-Presenter 281. `mission_transfer.php` ist eine 62-zeilige Kompatibilitätsfassade vor Export 67 und Import 339; alle sechs Module bleiben unter 400, die Layout-Ausnahme ist entfernt. Gezielte Fassaden-/Verhaltenstests: 50 Tests/261 Assertions. Kanonischer Integration-Ausschnitt `file-size,guard-harness`: `[2/2] pass`, der Harness beweist wieder alle positiven, negativen und Zero-Match-Fälle (`qa-e8-rest-file-size.json`) |
| `phpunit-full` konkurriert mit den laufenden QA-Workern um synthetische Deploy-Zeilen; `BulkVmActionsTest` traf dabei einen MySQL-Deadlock | Etappe-8-Rest, QA-Isolation | Erledigt. Kein globaler Retry in `repo_transaction()`: seine 53 Aufrufstellen umfassen nicht nur idempotente DB-Arbeit, sondern auch Auth-/Directory-/Auditpfade, sodass eine Wiederholung außertransaktionale Seiteneffekte duplizieren könnte. Keine neue Produktiv-Lockreihenfolge für ein ausschließlich vom Testharness erzeugtes Zweitbesitzermodell. Stattdessen stoppt `phpunit-full` Deploy- und Maintenance-Worker vor der Suite und startet beide im `finally` mit Compose `--wait`; ein roter Test kann spätere Gates daher nicht ohne Worker hinterlassen. Pester-Vertrag 2/2, live während der Vollsuite beide Worker gestoppt, kanonisch `qa-stack,phpunit-full,health-contract` `[3/3] pass` ohne Skips (`qa-e8-rest-worker-isolation.json`) |
| Ein vorheriger `docker exec` als root kann `/tmp/virtusphere-directory` vor Playwright mit falschem Owner anlegen; die als `www-data` laufenden AD-Specs enden dann reihenfolgeabhängig mit `tls_failed` | Etappe-8-Rest, QA-Isolation | Erledigt, ohne den richtigen Ownership-Check in `directory_ca_file()` zu lockern. Der QA-PHP-Service bindet den Cachepfad als eigenes tmpfs mit `uid=33,gid=33,mode=0700` ein; root kann den Mountpoint nicht ersetzen. Der aufgelöste Compose-Vertrag enthält die Optionen exakt, `DirectoryModuleContractTest` pinnt sie mit vollem Repo-Mount (9 Tests/49 Assertions, keine Skips). Im echten QA-Container blieb der Pfad vor und nach beiden Aufrufen `33:33:700`: root wurde mit `tls_failed` abgewiesen, `www-data` schrieb dieselbe CA erfolgreich. Danach lief die fokussierte Chromium-Suite einschließlich Setup und aller AD-Specs `[16/16]` grün (`16 passed`) |
| `DirectoryLdapFixtureFailoverTest` bricht mit einem Fehler ab, statt zu skippen, wenn die LDAP-Fixture fehlt | AD/LDAPS-Baseline, nicht Etappe 8 | Gemeldet, nicht von dieser Etappe behoben. `setUp()` skippt korrekt, aber `tearDown()` greift danach auf das nicht initialisierte `$db` zu; ohne die QA-Compose-Fixture erscheinen dadurch vier Errors statt vier Skips. Reproduziert vor und nach den Etappe-8-Änderungen mit identischer Ausgabe; in der Integration-Lane mit laufender Fixture läuft die Suite durch (`phpunit-full` grün ohne Skips). Während dieser Etappe ist im parallelen Arbeitsbaum eine fremde Korrektur entstanden (`isset($this->db)`-Guard in `DirectoryLdapFixtureFailoverTest` und `EsxiTrustModeIntegrationTest`); sie gehört nicht zur Eigentumsmenge von Etappe 8 und ist bewusst nicht Teil ihres Commits |
| 8R (dauerhafter Remote-Owner) ist innerhalb von Etappe 8 nicht umsetzbar | 8R als eigene Teiletappe | Offen und bewusst abgetrennt: 8R-1/8R-3 verlangen einen echten Ansible-Host mit systemd-User-Bus und Linger sowie ein kontrolliertes Migrationsfenster (8R-2). Ein solcher Host steht nicht zur Verfügung; die Sitzung baut nichts, was sie nicht beweisen kann. Etappe 8 bleibt damit planlegitim auf der direkten SSH-Kette, weil kein Modus 8R-aktiviert ist (Abschnitt 0, Punkt 1 der Remote-Erweiterung: erst ein aktivierter Modus verbietet den langlebigen direkten SSH-Step). Alle 8R-abhängigen Zusagen (Prepare/Launch/Poll/Reattach, `per_vm_async`-Create, vollständiger Cleanupzustand, Epoch-/Job-Fencing) bleiben unangetastet offen |
| Direkter Zugang zum Air-Gap-/Ansible-/ESXi-Ziel wird dauerhaft nicht bereitgestellt | 8R-O/8R-S-Vertrag, ADR-0040 | Implementierung wird nicht mehr vollständig blockiert und Standortreife nicht vorgetäuscht: 8R-O darf ausschließlich deaktivierte, fail-closed Produktbasis lokal beweisen; 8R-S bleibt Owner aller echten Host-, Mess-, Fault-, Beobachtungs- und Aktivierungsnachweise. Ohne 8R-S bleibt jede neue Remote-Moduszeile `disabled`; Create/Full zusätzlich bis 14B gesperrt |
...
| Ansible-Inventory/-Command mischen Parser-, Artifact-, Mode-, Marker- und Preflightdomänen auf 714/523 Zeilen | 7/8 Strukturhunks | Beide Anteile erledigt. Etappe 7 zerlegte `ansible_inventory.php` (714 → 38 Zeilen Fassade plus vier Domänenmodule). Etappe 8 zerlegte `ansible_command.php` (523 → 24 Zeilen Fassade plus `ansible_command_shell.php` 17, `ansible_command_modes.php` 269, `ansible_command_preflight.php` 312). Der Strukturhunk lief vor der fachlichen Änderung: alle 18 öffentlichen Funktionen sind nach isoliertem Require nur der Fassade in eigenem Prozess vorhanden, jede hat genau einen Owner, und `ansible_sh_quote()` existiert genau einmal im gesamten `lib/` (eine zweite Quoting-Regel wäre ein Command-Injection-Unterschied, den niemand bemerkt, solange beide Dateien richtig aussehen). Owner-Registry `VIRTUSPHERE_ANSIBLE_COMMAND_MODULES` (`lib/ansible_command_modules.php`), beidseitig gegen das Dateisystem geprüft von `AnsibleCommandModuleContractTest`; `DeployConvergenceContractTest` und `AnsiblePlaybookHygieneContractTest` lesen jetzt diese Registry statt des historischen Dateinamens, der nach dem Split nur noch eine Require-Liste ist und ihre Zusicherungen leer hätte durchlaufen lassen. `ansible_command.php` ist aus `FILE_SIZE_ALLOWANCES` entfernt |
