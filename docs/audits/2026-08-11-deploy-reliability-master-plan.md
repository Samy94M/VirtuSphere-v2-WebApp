...
| `file-size`-Gate ist bereits auf `HEAD` rot, unabhängig von dieser Etappe | Verursacher `3187391` (Mission-Import), nicht Etappe 8 | Offen, ausdrücklich nicht in dieser Etappe behoben. Der Commit vor Etappe 8 ließ `lib/layout.php` von 666 auf 706 Zeilen wachsen (Ausnahme ist eine Decke, keine Zielgröße) und legte `lib/mission_transfer.php` mit 457 Zeilen ohne Ausnahme an. Beides ist gegen `git show HEAD:` verifiziert und stammt aus fremder Arbeit; Etappe 8 hat jede eigene Datei unter der Grenze gehalten und dafür `deploy_job_output.php` und `deploy_job_log_spool.php` abgetrennt sowie das Kontingent von `migrate.php` wie bei jeder Migration zuvor auf die neue Zeilenzahl gesetzt. `layout.php` gehört Etappe 13, `mission_transfer.php` seinem eigenen Strang |
