...
**Status und nächste Reihenfolge:** Q3 entschieden. T1/T2 und der serverseitige P5-Kern sind am 2026-09-22 umgesetzt: geschlossene V1-Validierung und gemeinsame Fixture, Migration 0056 samt konvergentem Fresh-Schema, transaktionaler Writer, permanente Minimalmarker, VM-/Restore-Generationen und die additive Action `reportPackageRun`. DB- und echte HTTP-Integration beweisen Deduplizierung ohne Fristverlängerung, Konflikt-Rollback, 256 normale Details plus späten ersten Fehler und Abschluss, Ablauf, Restore-Fence, eindeutige MAC-Zuordnung, IP-Allowlist, getrenntes 64-KiB-Limit und korrelierte Antworten. `getDeviceInfos` liefert die zwei Generationen additiv und `client_getinfo.ps1` validiert und publiziert sie im Snapshot. Noch offen sind Maintenance-Bereinigung, Restore-Rotation im unterstützten Workflow sowie T3/T4 Reporter, vollständige Verteilung und reale Zeitbudgetmessung; danach folgen P7/P8 Portal und gemeinsame Abnahme. Keine weitere Nutzerfrage ist dafür offen.
