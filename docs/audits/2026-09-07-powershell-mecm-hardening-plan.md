...
| A14 | lokal verifiziert / externe Abnahme offen; Löschanforderung und Ersatzverteilgrenze durch ADR-0034 Amendment 6 abgelöst | Ohne `removeOldVersion=true` behalten Importläufe Altobjekte. Mit diesem JSON-Flag führt der Autoimporter den unmittelbar revalidierten Schema-3-Plan automatisch aus: alte Deployments, Applications und Collections werden in dieser Reihenfolge entfernt. Kanonische punktgetrennte Dezimalversionen werden ohne Integerüberlauf ordinal verglichen; freie/mehrdeutige Versionen blockieren. Vollständiger Quell- und Referenzscan, exakte Marker, referenzfreie Kandidaten, bestätigter und an die Content-ID gebundener Ersatzauftrag, sichere Zielevidenz sowie neues Deployment bleiben Pflicht. Vollständiger DP-Erfolg oder mindestens ein erfolgreicher DP sind keine Voraussetzung. Planhash und Bereitschaft werden unmittelbar vor dem ersten Einzelschritt erneut geprüft; Teilfehler stoppen den Rest mit sichtbarem `[n/total]`. | Historischer Beleg: Version-/Eigentums-/Referenz-/Stale-Plan-/Teilfehlervektoren im damaligen Abschlusszwischenlauf grün; `[2/2] pass`, 569/569 Pester, Coverage 80,5 %, Analyzer ohne Befund. Aktuelle Tests ergänzen bestätigten Ersatz mit null erfolgreichen DPs, Schema-1/2-Ablehnung, Deploymentreihenfolge/-hash, Referenzen und unvollständige IDs. | Schema-3-Plan gegen echte MECM-Referenzen und offene DP-Verteilung prüfen; in einem Stagingpaket `removeOldVersion=true` setzen, automatische Entfernung beobachten und einen kontrollierten Teilfehler verifizieren. |
...
Die Planerstellung verlangt aktuell keine Rückfrage. Für die Umsetzung gelten die konservativen Ausgangsregeln dieses Dokuments. Die Löschpolitik für Altbestand ist inzwischen durch ADR-0034 Amendment 6 entschieden: ausschließlich `removeOldVersion=true` fordert den automatischen, revalidierten Schema-3-Plan an. Andere neue Produktentscheidungen, etwa automatische Adoption ungeklärter MECM-Regeln, weiterhin zuerst mit Alternativen und Folgen vorlegen; unabhängige Pflichtreparaturen weiterführen.
