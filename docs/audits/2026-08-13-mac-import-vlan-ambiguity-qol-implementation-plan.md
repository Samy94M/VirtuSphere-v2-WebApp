...
Der Gesamtplan ist erst abgeschlossen, wenn zusätzlich zu Abschnitt 15 alle Punkte belegt sind. Die folgende Einzelbewertung ersetzt die ungepflegten Checkboxen. Sie unterscheidet **erledigt**, **teilweise erledigt**, **extern blockiert** und **offen**. Eine lokale Implementierung ist keine Standortfreigabe.

Nachpflege vom 2026-09-08, geprüft gegen Ausgangscommit `e1c57d33ea463731f02b310e614e9bc779e82209`, aktuelle Owner, ADR-0040/0041/0042 und die Abnahmezeilen 8R-O, 13/13R, 14A, 14B, 14C sowie Gesamtabgleich des Masterplans. Pfade mit `lib/` und `tests/` beziehen sich in dieser Tabelle auf `Docker/WebAPI/`. Die genannten Tests sind dauerhafte prüfbare Verträge; historische grüne Läufe bleiben an ihren protokollierten Commit gebunden.

| Nr. | Anforderung | Status | Belegte Evidenz und exakt verbleibender Anteil |
|---|---|---|---|
| 1 | Masterplan enthält 8R, 13R, 14A, 14B, 14C in der vereinbarten Reihenfolge | erledigt | Masterplan Abschnitt 1.1 und Abnahmezeilen 8R-O, 13 mit 13R, 14A, 14B, 14C; 8R-O/8R-S-Trennung ist ausdrücklich erhalten. Reihenfolge und Scope sind dokumentiert, keine Aktivierung behauptet. |
| 2 | Self-Healing-Reviewspur und widerspruchsfreier Create-Plan | erledigt | Statuskopf des Self-Healing-Plans nennt ausdrücklich „Reviewspur … nicht separat ausführen“. Create-Plan und Masterplan-Abnahme 14B (`c02725e`) halten die Abweichung `ExitType=cgroup` als 8R-Aufgabe fest; die allgemeinen Remote-Regeln dieses Plans haben Vorrang. |
| 3 | DB-Handle, Run-Token, Unit/Cgroup und Marker vor jeder Remote-Mutation | teilweise erledigt | `repo_prepare_remote_inventory_execution()` und `remote_inventory_identifiers()` in `lib/repo/deploy_remote_execution.php`, Runnerprotokoll unter `Ansible/runner/`, `RemoteInventoryConsumerTest`; 8R-O-3 (`5279243`). Bewiesen ist die deaktivierte Grundlage. Produktverdrahtung aller mutierenden Modi und echte Unit/Cgroup-Nachweise fehlen in 8R-S. |
| 4 | Vier getrennte, exhaustiv geprüfte Zustandsachsen | teilweise erledigt | `remote_execution_state_edges()` und `remote_inventory_observation()`, `RemoteExecutionProtocolTest`, `RemoteRecoveryPolicyTest` trennen Controller, Wirkung, Reconciliation und Cleanup. Die vollständige reale Faultmatrix über alle Modi ist nicht durch diese lokalen Zustandsproben ersetzt; sie bleibt 8R-S. |
| 5 | Epoch und Jobtoken fencen jeden Write und nächsten Step | teilweise erledigt | `repo_assert_remote_execution_fence()` und `RemoteInventoryConsumerTest` prüfen Token, Epoch, Attempt und Generation der Inventargrundlage; 8R-O-3-Abnahme. Kein Nachweis für die allgemeine Umschaltung sämtlicher Legacy-Produktwrites; Aktivierung und Fehlerfälle am echten Ziel fehlen. |
| 6 | Unveränderlicher Vertrag/Generation je Job und explizite Modusaktivierung | teilweise erledigt | Migration `0042_remote_execution_foundation`, `RemoteExecutionFoundationContractTest`, `RemoteActivationMaterializationTest` und 8R-O-2 (`258b6d7`) belegen additive Felder und explizites `disabled` für neue Moduszeilen. Historische Legacy-/NULL-Verträge werden nicht nachträglich umgedeutet; die Aussage „jeder Job vor Ausführung“ im allgemeinen Remote-Produktpfad bleibt unbewiesen. |
| 7 | Reaper terminalisiert keine möglicherweise aktive Remote-Ausführung | teilweise erledigt | `remote_recovery_decision()`, `repo_request_remote_recovery()`, `repo_remote_safe_vm_sweep_candidates()` und `RemoteRecoveryFoundationTest` schützen die deaktivierte Recoverygrundlage. Die Masterplan-Abnahme 8R-O-4 (`0a0d5e5`) hält die fehlende gemeinsame Verdrahtung von Reaper, Consumer und Sweep ausdrücklich fest. |
| 8 | Nach Worker-/SSH-/DB-/Containerverlust gleicher Run oder ehrliche Eskalation | teilweise erledigt | `RemoteInventoryConsumerTest`, `DeployWorkerDbChannelRecoveryTest`, `DeployCreateWorkerFlowTest` und `DeployCreateRetryMatrixTest` prüfen lokale Fences, Reattach und unsichere Create-Ergebnisse. Reale Transport-, Hostrestart- und ESXi-Effektnachweise über alle Modi fehlen in 8R-S/14B-Standortabnahme. |
| 9 | Nonzero, Signal und Timeout sind kein Beweis für Nichtmutation | teilweise erledigt | `remote_inventory_observation()` setzt bei Ergebnissen den Effekt nicht auf Nichtstart zurück; `RemoteExecutionProtocolTest` und die Create-Ergebnismatrix halten Unsicherheit getrennt. Echte mutierende Zielschritte mit Signal-/Timeoutfehlern bleiben extern nachzuweisen. |
| 10 | Restore erneuert Generation vor Claim und inventarisiert fremde Generationen read-only | offen | `lib/repo/deploy_runtime_identity.php` liest die bestehende Generation. Der aktuelle `scripts/restore_test.sh` prüft Restore, Schema und APP_KEY, enthält aber keinen Nachweis der geforderten Generationserneuerung oder Remote-Fremdgenerationsinventur. Initiale zufällige Generation aus Migration 0042 und der frühere Restore-Drill schließen diese Anforderung nicht. Umsetzung und anschließend 8R-S-Drill fehlen. |
| 11 | Secret-Cleanup früher als Evidenzretention, kein Beweisverlust | teilweise erledigt | Runner-Redaction, `repo_begin_remote_inventory_cleanup()`/`repo_record_remote_inventory_cleanup()` und `RemoteInventoryConsumerTest` belegen lokale Secret-/Cleanup-Barrieren. Abschnitt 24.2 lässt standortabhängige Retentionwerte bewusst offen; realer Secret-Lebenszyklus und Verlustfreiheit über das Ziel-Beobachtungsfenster fehlen. |
| 12 | Sichere, append-only manuelle Klärung ohne Wegklicken von unknown | erledigt | `lib/system_status_service_actions.php`, `lib/repo/deploy_job_recovery_actions.php` und Masterplan-Abnahme 13/13R: RBAC/CSRF/Confirm, append-only `deploy_recovery_resolutions`, Prüfung dokumentiert Operatoraussage und schreibt keinen erfundenen Remote-Erfolg. Create-Freigabe verlangt zusätzliche frische Inventarevidenz (`DeployCreateReleaseTest`). |
| 13 | Drei Achsen aus einem Dienstsnapshot | erledigt | `deploy_service_health_snapshot()` in `lib/deploy_service_health.php`, `DeployServiceHealthTest`, Masterplan-Abnahme 13/13R. Dashboard, Deploy, Systemstatus und Health konsumieren denselben Snapshot; Detailansichten behalten Verfügbarkeit, Claim-Freigabe und Recovery-Aufmerksamkeit getrennt. |
| 14 | Supervisor erst nach Durable-Freigabe und bestätigtem Ende des Kindes | extern blockiert | `deploy_supervisor_decide()`, `deploy_supervisor_after_exit()`, `deploy_supervisor_switch_blockers()`, `DeploySupervisorFaultRunTest` und 14C (`81fb19e`) belegen den lokalen Prozessvertrag. `supervisor_v1` bleibt inaktiv. Reale Betriebsfreigabe aller produktiven Modi und Supervisorbetrieb am Ziel fehlen; lokale Kindprozessproben autorisieren keine Aktivierung. |
| 15 | supervisor_v1 schließt Legacy-Claims und implizit aktive neue Modi aus | teilweise erledigt | `lib/repo/deploy_supervisor_contract.php`, `DeploySupervisorContractTest` und `RemoteActivationMaterializationTest` belegen fail-closed Umschalt-/Materialisierungsregeln. Laufender produktiver `supervisor_v1` mit freigegebenen Zielmodi ist nicht abgenommen. |
| 16 | Grüne Recovery-/Faultpolicy für alle sieben Modi | teilweise erledigt | `lib/remote_step_policy.php`, `RemoteStepPolicyTest`/`RemoteStepPolicyContractTest` und 8R-O-5 (`f9197ec`) decken Inventory, Export, Start, Autostart und Powercycle als deaktivierte Policies ab. Create/Full besitzen ihren separaten ADR-0041-Unitpfad. Eine grüne gemeinsame reale Recovery-/Faultabnahme je Modus fehlt in 8R-S. |
| 17 | Create-JID, UUID/MOID und Ergebnis am selben Job/Attempt/Remotehandle | teilweise erledigt | `DeployCreateResultsIntegrationTest`, `DeployCreateWorkerFlowTest`, `DeployCreateReleaseTest`, ADR-0041 und Abnahme 14B belegen Job-/Attempt-/Unitbindung, JID und Live-Identität. Allgemeiner Durable-Remotehandle/Cgroup-Vertrag und echter Standalone-ESXi-Verlustfall sind keine lokal abgeschlossene 14B-Eigenschaft. |
| 18 | Feste Präzedenz Remote, Identity, Netzwerk/MAC, externe Voraussetzungen | erledigt | `deploy_retry_blockers()` in `lib/repo/deploy_job_retry.php`, `DeployJobRetryFlowTest`, `NetworkMacContractTest` und 14A-Abnahme (`0536a67`) pinnen Reihenfolge und Schutz vor Scope-Erweiterung bei Partial-Ergebnissen. |
| 19 | Job-115-Fixture: Remote-Erfolg, partial, genau drei ambiguous_vlan | offen | Abschnitt 16.1 dokumentiert den historischen Vorfall; `MacImportCallbackTest` prüft Ambiguität, aber die Suche im aktuellen Testbestand findet keine explizite Job-115-Fixture mit genau dieser Kardinalität und kombinierter Remote-/Fachzustandsaussage. Historischer Job und allgemeine Callbacktests ersetzen diese Probe nicht. |
| 20 | Terminale Jobs ohne kontrollierte Linux-Prozessgruppe; laufende vSphere-Tasks sichtbar ungeklärt | extern blockiert | ADR-0040, ADR-0041, `DeployCreateResultStateTest` und Cleanup-Barrieren belegen lokale Unsicherheitsdarstellung. Masterplan-Abnahme 14B nennt `ExitType=cgroup` ausdrücklich als nicht implementierte 8R-Aufgabe. Prozessgruppen-/Task-Endnachweis auf dem echten Ansible-/ESXi-Ziel fehlt. |
| 21 | Queue/Pause/Recovery/Cooldown/Offline/Manual Review ohne Ubuntu-Zugriff bedienbar | teilweise erledigt | Dienstsnapshot, `system_status_handle_service_action()`, `DeployRecoveryHelpContractTest` und Masterplan-Abnahmen 13/13R/14C belegen Portalaktionen und Erklärungen. Menschlicher Screenreader, reale Störungsbedienung und Standort-Recovery sind weiterhin ausstehende Abnahmen. |
| 22 | Alle Quellen-/SSoT-/Bounds-/Schema-/Wire-/Portal-/Doku-/Air-Gap-Gates grün | teilweise erledigt | Masterplan protokolliert den historischen grünen Release-Lauf auf `2d5809d` (`qa-artifacts/qa-abschluss-release3.json`, Commit `e5e2904`). Der spätere Lauf auf `7fe9b18` war mit WebKit- und Dirty-Bundle-Befund rot; er bleibt rot dokumentiert. Ein neuer gemeinsamer Nachweis auf dem Abschlusscommit dieses Restauftrags steht noch aus. |
| 23 | Alle Fault-/Browser-/Accessibility-/Restore-/Rollback-/Stagingdrills grün mit Fortschritt | extern blockiert | Masterplan-Abnahme „Release-Lane/Staging-Drill“ trennt lokale Drills und echte Hardwareabnahmen. Es fehlen weiterhin 8R-S samt User-Bus/Linger/Cgroup, reale Ansible-/Standalone-ESXi-Faultmatrix, MECM-Lab/Cutover, Ziel-AD, Windows-SYSTEM und menschlicher Screenreader. Lokale Browser-, LDAP-, Prozess- und Restore-Fixtures ersetzen diese Nachweise nicht. |

Die historischen Abnahmezeilen und Messwerte werden durch diese Nachpflege nicht umgeschrieben. Insbesondere bedeutet der frühere grüne „Gesamtabgleich“ keinen Vollabschluss dieser kombinierten DoD. Die zusätzlich konkretisierten lokalen Lücken 10 und 19 gehören vor einer allgemeinen Remote-Freigabe in die Wiederaufnahme von 8R; sie werden in diesem Dokumentationspaket nicht durch Aktivierung oder fingierte Zielnachweise geschlossen.
