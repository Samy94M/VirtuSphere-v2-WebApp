...
- `db_importMAC.php?action=updateInterface` requires the new payload shape `{ "mission_id": 123, "results": [...] }`. Imports that reach a VM but do not match an interface VLAN return `success:false`, keep `updated_interfaces` at `0`, and include `unmatched_interfaces[].status = "error"` with the VM name and VLAN.
