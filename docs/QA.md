...
There are two ways to answer a fixable finding and only one of them is usually right. An exception in `.trivyignore.yaml` is for a finding with **no** available fix; when a fixed package version exists, it belongs in the image. Both project-built images already carry that answer as a line of their own: `Docker/nginx/Dockerfile` upgrades the Alpine packages the pinned base lags on (`libcrypto3 libssl3 libexpat libuuid`), and `Docker/mysql/Dockerfile` runs `microdnf update` and removes MySQL Shell with its bundled Python. A base-image digest bump is a third, weaker move that is worth checking but frequently does not help on its own: measured in 2026-09, the newest published `nginx:1.31-alpine` still shipped util-linux `2.42.1-r0` while the seven fixable findings needed `2.42.3-r1`, so the bump alone would have left the gate red.
