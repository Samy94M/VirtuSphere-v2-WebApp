...
There are two ways to answer a fixable finding and only one of them is usually right. An exception in `.trivyignore.yaml` is for a finding with **no** available fix; when a fixed package version exists, it belongs in the image. All three project-built images carry that answer as a line of their own: `Docker/nginx/Dockerfile` upgrades the Alpine packages the pinned base lags on (`libcrypto3 libssl3 libexpat libuuid`), `Docker/mysql/Dockerfile` runs `microdnf update`, removes MySQL Shell with its bundled Python and rebuilds `gosu` against a current Go toolchain, and `Docker/phpmyadmin/Dockerfile` updates the bundled `twig/twig` and `symfony/cache` in a builder stage and copies only the resulting vendor tree forward. A base-image digest bump is a third, weaker move that is worth checking but frequently does not help on its own: measured in 2026-09, the newest published `nginx:1.31-alpine` still shipped util-linux `2.42.1-r0` while the seven fixable findings needed `2.42.3-r1`, so the bump alone would have left the gate red.
...
**Both gates that seed and delete deploy rows quiesce the QA workers first**, and for the same reason: `phpunit-full` and every Playwright gate create queued/running/stale jobs on purpose and then delete their fixtures, so a concurrently claiming deploy worker or reaping maintenance worker is a second owner of the same throwaway rows. `phpunit-full` has done this since the Etappe-8 remainder, after `BulkVmActionsTest` hit a real MySQL deadlock. `Invoke-PlaywrightSuite` did not, and on 2026-09-06 the Release lane showed why: `e2e-portal` failed with `Deadlock found when trying to get lock` inside the `beforeEach` cleanup of `deploy-actions.spec.js`, right after its retry test had created the one job that file's own header says the worker may claim. The assertions never depended on that job's fate; the cleanup's `DELETE FROM deploy_jobs` did. No spec needs a live worker: with both stopped the full Chromium suite is 259/259 and finishes faster than with them. Production locking is unchanged, and a `finally` restarts both with `--wait` even after a red or throwing run.
...
That check found one on its first run, and the leftover had a cause worth knowing. `field-roundtrip.spec.js` cleaned up with `DELETE ... WHERE mission_name LIKE 'e2ert-%'`, which silently left its emoji probe behind on every run: `mission_name` is `utf8mb4_unicode_ci` with a unique index, and MySQL plans `LIKE 'prefix%'` on it as an index **range** once the table is big enough. A supplementary character (U+10000 and above) carries an implicit collation weight the range endpoints do not represent, so the row falls outside the computed range and is silently missed.

Two details of that were written down wrongly here at first and were corrected after re-measuring while closing the same finding for migration `0050`. **It is not a property of DELETE.** A SELECT taking the same range loses exactly the same rows; measured over 36 rows, the range answers 19 of 21 while `IGNORE INDEX` and `LEFT()` both answer 21. What made it look statement-specific is size: a small fixture gets a full index scan and looks healthy, which is why nothing below the optimizer's switching point can see this. **And the position decides, not the mere presence of the character.** `e2ert-🚀…` is missed because the emoji sits immediately after the prefix and takes part in the boundary comparison; `_Vorlage🚀…` is found, because the second character already settles it. That is also why exactly one of `field-roundtrip.spec.js`'s seven probes was affected. BMP characters are unaffected, and that includes an emoji with a variation selector such as U+2714 U+FE0F, which an earlier version of this paragraph listed as affected. `utf8mb4_bin` behaves like `utf8mb4_unicode_ci` here, so a binary name column is no safer.

The cleanup now compares with `LEFT(mission_name, CHAR_LENGTH(?))`, which defeats the index so the predicate is evaluated per row. `tests/Static/PrefixMatchContractTest.php` keeps the shape out of `lib/`.
